...
- 1+ nodos en ejecución y completamente iniciados en la Testnet `Fuji`. Consulta la sección
...
Con más datos en `Fuji`, puede llevar tiempo iniciar la Testnet `Fuji` desde cero.
Puedes usar [State-Sync](/nodes/configure/chain-config-flags.md#state-sync-enabled-boolean)
para acortar el tiempo de inicio.
...
alguna validación para reducir problemas debido a una entrada no válida. La primera pregunta pide el tipo de
...
? ¿Cómo te gustaría establecer las tarifas:
  ▸ Uso bajo de disco    / Baja capacidad de procesamiento    1.5 mil gas/s (configuración de C-Chain)
    Uso medio de disco / Capacidad de procesamiento media 2 mil   gas/s
    Uso alto de disco   / Alta capacidad de procesamiento   5 mil   gas/s
...
`Uso bajo de disco / Baja capacidad de procesamiento 1.5 mil gas/s` para este tutorial.

La siguiente pregunta es sobre el airdrop:

```bash
✔ Uso bajo de disco    / Baja capacidad de procesamiento    1.5 mil gas/s
Utiliza las teclas de flecha para navegar: ↓ ↑ → ←
...
secuencia de preguntas y el comando finaliza:
...
Además, este tutorial asume que un nodo está en funcionamiento, completamente iniciado en `Fuji`, y se ejecuta
...
Las Subredes están actualmente con permisos únicamente. Por lo tanto, el proceso ahora requiere que el usuario proporcione
...
red primaria, lo que significa que tu nodo ha **completado completamente el arranque**.
...
poder acceder al binario de complemento correspondiente. Como este es el comando `join`, que no
sabe aún sobre el complemento, es necesario proporcionar el directorio donde reside el complemento. Asegúrate de proporcionar
