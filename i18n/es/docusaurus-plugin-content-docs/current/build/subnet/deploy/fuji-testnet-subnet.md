...
y puedes obtener el token de prueba desde el [faucet](https://faucet.avax.network/).
...
`Avalanche-CLI` soporta las siguientes operaciones de clave:
...
Para implementar en `Mainnet`, sigue [este tutorial](/build/subnet/deploy/mainnet-subnet.md).
...
financiar tu clave desde el [faucet](https://faucet.avax.network/). El comando también imprime las direcciones de la P-Chain
...
avalanche key list
+-----------+-------------------------------+-------------------------------------------------+---------------+
| NOMBRE DE  |             CADENA            |                     DIRECCIÓN                    |    RED        |
|    LA     |                               |                                                 |               |
|   CLAVE   |                               |                                                 |               |
+-----------+-------------------------------+-------------------------------------------------+---------------+
...
La pregunta final está preguntando por precompilaciones. Las precompilaciones son personalizaciones poderosas de tu EVM. Lee sobre ellas en [precompilaciones](/build/subnet/upgrade/customize-a-subnet.md#precompiles).
...
Para este tutorial, supongamos el caso simple de no tener precompilaciones adicionales. Esto finaliza la secuencia de preguntas y el comando se cierra:
...
Finalmente, es necesario definir el umbral de cuántas claves se requieren para que un cambio sea válido
-hay alguna validación de entrada-. Por ejemplo, si hay una clave de control, como se mencionó anteriormente, solo ingresa 1.
...
```bash
La Subred se ha creado con ID: 2EkPnvnDiLgudnf8NjtxaNcVFtdAAnUPvaoNBrc9WG5tNmmfaK. Ahora creando la blockchain...
...
Bien hecho. Acabas de crear tu propia Subnet con tu propia Subnet-EVM funcionando en `Fuji`.
...
Proporciona una ruta a un archivo de configuración. Si ejecutas este comando en la máquina donde tu
validador está en funcionamiento, entonces podrías apuntar esto al archivo de configuración utilizado realmente, por ejemplo
`/etc/avalanchego/config.json` - solo asegúrate de que la herramienta tenga permisos de **escritura** en el archivo. O
...
conoce aún sobre el plugin, es necesario proporcionar el directorio donde reside el plugin. Asegúrate de proporcionar la ubicación para tu caso:
