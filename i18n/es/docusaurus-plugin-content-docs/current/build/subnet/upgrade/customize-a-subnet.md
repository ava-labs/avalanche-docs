...
para evitar colisiones de ID, reservar y publicar adecuadamente su ID de Cadena.
...
_Esto es utilizado por otras precompilaciones. Consulte las secciones de precompilación para ver cómo funciona esto._
...
desplegador válido en lugar del llamador de `CREATE`. Esto significa que los contratos de fábrica aún podrán
...
Esto permitirá que solo `0x8db97C7cEcE249c2b98bDC0226Cc4C2A57BF52FC` despliegue contratos. Para más
información sobre configuraciones iniciales de precompilación, consulte [Configuraciones iniciales de precompilación](#configuraciones-iniciales-de-precompilación).
...
Es posible habilitar esta precompilación con una configuración inicial para activar su efecto en
la marca de tiempo de activación. Esto proporciona una forma de habilitar la precompilación sin una dirección de administrador para gestionar
la lista de transacciones permitidas. Con esto, puede definir una lista de direcciones que tienen permitido enviar
...
Esto permitirá que solo `0x8db97C7cEcE249c2b98bDC0226Cc4C2A57BF52FC` envíe transacciones. Para más
información sobre configuraciones iniciales de precompilación, consulte [Configuraciones iniciales de precompilación](#configuraciones-iniciales-de-precompilación).
...
Esto establecerá la configuración de tarifas con los valores especificados en el campo `initialFeeConfig`. Para obtener más información sobre las configuraciones iniciales de los precompilados, consulte [Configuraciones iniciales de precompilados](#initial-precompile-configurations).

### Cambiando los mecanismos de recompensa de tarifas

El mecanismo de recompensa de tarifas se puede configurar con este contrato de precompilado estatal llamado `RewardManager`. La configuración puede incluir quemar tarifas, enviar tarifas a una dirección predefinida o habilitar que las tarifas sean recolectadas por los productores de bloques. Este precompilado se puede configurar de la siguiente manera en el archivo génesis:
...
`adminAddresses` denota las cuentas de administrador que pueden agregar otras cuentas `Admin` o `Enabled`. Tanto `Admin` como `Enabled` son elegibles para cambiar el mecanismo de tarifas actual.

El precompilado implementa la interfaz `RewardManager`, que incluye la interfaz `AllowList`. Para un ejemplo de la interfaz `AllowList`, consulte [TxAllowList](#allowlist-interface) arriba.

El contrato de precompilado `Stateful Precompile` que alimenta el `RewardManager` se adhiere a la siguiente interfaz de Solidity en `0x0200000000000000000000000000000000000004` (puede cargar esta interfaz e interactuar directamente en Remix). También se puede encontrar en [IRewardManager.sol](https://github.com/ava-labs/subnet-evm/blob/5faabfeaa021a64c2616380ed2d6ec0a96c8f96d/contract-examples/contracts/IRewardManager.sol):
...
El precompilado `RewardManager` utiliza la interfaz `IAllowList` directamente, lo que significa que utiliza las mismas funciones de la interfaz `AllowList` como `readAllowList` y `setAdmin`, `setEnabled`, `setNone`. Para obtener más información, consulte [Interfaz de Solidity de AllowList](#allowlist-interface).
...
- `allowFeeRecipients` - permite a los productores de bloques reclamar tarifas. Esto permitirá a los productores de bloques reclamar tarifas especificando sus propias direcciones en sus configuraciones de cadena. Consulte [aquí](#fee-recipient) para obtener más información sobre cómo especificar la dirección del receptor de tarifas en la configuración de la cadena.

- `disableRewards` - deshabilita las recompensas de bloque y comienza a quemar tarifas.

- `currentRewardAddress` - devuelve la dirección de recompensa actual. Esta es la dirección a la que se envían las tarifas. Puede incluir una dirección de agujero negro (`0x010...0`), lo que significa que las tarifas se queman. También puede incluir un hash predefinido (`0x0000000000000000000000000000000000000000`) que denota que se permiten receptores de tarifas personalizados. Se recomienda utilizar la función `areFeeRecipientsAllowed` para verificar primero si se permiten receptores de tarifas personalizados.

- `areFeeRecipientsAllowed` - devuelve true si se permiten receptores de tarifas personalizados.

Estos 3 mecanismos (quemar, enviar a una dirección predefinida y habilitar que los productores de bloques recojan tarifas) no se pueden habilitar al mismo tiempo. Habilitar un mecanismo tomará el control sobre el mecanismo anterior. Por ejemplo, si habilita `allowFeeRecipients` y luego habilita `disableRewards`, `disableRewards` tomará el control y las tarifas se quemarán.

_Nota: Las direcciones de recompensa o direcciones de receptores de tarifas no necesitan ser cuentas de administrador o habilitadas._

#### Configuración inicial

Es posible habilitar este precompilado con una configuración inicial para activar su efecto en la marca de tiempo de activación. Esto proporciona una forma de habilitar el precompilado sin una dirección de administrador para cambiar el mecanismo de recompensa de tarifas. Esto puede ser útil para redes que requieren un cambio de mecanismo de recompensa de una sola vez sin especificar ninguna dirección de administrador. Sin esta configuración inicial, el precompilado heredará el mecanismo de `feeRecipients` activado en el génesis. Esto significa que si `allowFeeRecipients` se establece en verdadero en el archivo génesis, el precompilado se habilitará con el mecanismo `allowFeeRecipients`. De lo contrario, seguirá quemando tarifas. Para usar la configuración inicial, debe especificar el mecanismo de recompensa inicial en el campo `initialRewardConfig` en su archivo génesis o de actualización.
...
Para establecer una dirección para recibir todas las recompensas de transacción, debe especificar el campo `rewardAddress` en el `initialRewardConfig`:
...
Para deshabilitar las recompensas y comenzar a quemar tarifas, debe dejar todos los campos en el `initialRewardConfig` vacíos:
...
Sin embargo, esto es diferente al comportamiento predeterminado del precompilado. Si no especifica el campo `initialRewardConfig`, el precompilado heredará el mecanismo `feeRecipients` activado en el génesis. Esto significa que si `allowFeeRecipients` se establece en verdadero en el archivo génesis, el precompilado se habilitará con el mecanismo `allowFeeRecipients`. De lo contrario, seguirá quemando tarifas. Ejemplo de configuración para este caso:
...
`blockTimestamp` debe establecerse en una marca de tiempo posterior a la fecha de Durango. `quorumNumerator` es el porcentaje de participación de los validadores que deben firmar un mensaje de Warp para que se considere válido. Debe establecerse en un valor entre 33 y 100. El valor predeterminado es 67. La precompilación `warpConfig` se puede desactivar más tarde estableciendo `disable` en `true` en el archivo upgrade.json.

Si desea usar el mensajero Warp en una cadena Subnet-EVM existente, debe coordinar una actualización con `upgrade.json`. Consulte [Actualizaciones de red: habilitar/deshabilitar precompilaciones](#actualizaciones-de-red-habilitardeshabilitar-precompilaciones) para obtener más información.

:::warning

Actualmente, Warp Precompile solo se puede activar en Fuji después de que ocurra Durango. Durango en Fuji está programado para las 11 a. m. ET del 13 de febrero de 2024. Si planea usar el mensajero Warp en su propia cadena Subnet-EVM en Fuji, debe actualizar a `subnet-evm@v0.6.0-fuji` y coordinar su actualización de precompilación. La configuración de Warp's "blockTimestamp" debe establecerse después de la fecha de Durango (13 de febrero de 2024, 11 a. m. ET).

:::

## Ejemplos de contratos

Subnet-EVM contiene contratos de ejemplo para precompilaciones en `/contracts`. Es un proyecto de hardhat
con pruebas y tareas. Para obtener más información, consulte
[README de ejemplos de contratos](https://github.com/ava-labs/subnet-evm/tree/master/contracts#subnet-evm-contracts).

## Actualizaciones de red: habilitar/deshabilitar precompilaciones

:::warning

Realizar una actualización de red requiere coordinar la actualización en toda la red. Una actualización de red
cambia el conjunto de reglas utilizado para procesar y verificar bloques, de modo que cualquier nodo que se actualice incorrectamente
o no se actualice a tiempo para que esa actualización entre en vigor puede quedar fuera de sincronización con el resto
de la red.

Cualquier error en la configuración de las actualizaciones de red de la red o en su coordinación en los validadores puede hacer que la red
se detenga y la recuperación puede ser difícil.

:::

Además de especificar la configuración para cada una de las precompilaciones anteriores en la configuración de la cadena génesis,
se pueden habilitar o deshabilitar individualmente en un momento dado como una actualización de red.
Deshabilitar una precompilación desactiva la llamada a la precompilación y destruye su almacenamiento para que pueda ser habilitada
en un momento posterior con una nueva configuración si se desea.

Estas actualizaciones deben especificarse en un archivo llamado `upgrade.json` ubicado en el mismo directorio donde
reside [`config.json`](#avalanchego-chain-configs): `{chain-config-dir}/{blockchainID}/upgrade.json`.
Por ejemplo, la actualización de `WAGMI Subnet` debe colocarse en
  ...
  "precompileUpgrades": [
    {
      "[NOMBRE_PRECOMPILACIÓN]": {
        ...
        "[PARÁMETRO]": "[VALOR]" // opciones de configuración específicas de la precompilación, p. ej. "adminAddresses"
      }
    }
  ]
}
...
Una `blockTimestamp` no válida en un archivo de actualización hace que la actualización falle.
...
Cada elemento en `precompileUpgrades` debe especificar exactamente una precompilación para habilitar o deshabilitar y las
marcas de tiempo de bloque deben estar en orden creciente. Una vez que se ha activado una actualización (se ha aceptado un bloque después de la
marca de tiempo especificada), siempre debe estar presente en `upgrade.json` exactamente como estaba configurada en el momento de la activación
(de lo contrario, el nodo se negará a iniciar).

Habilitar y deshabilitar una precompilación es una actualización de red y siempre se debe hacer con precaución.

:::danger

Por seguridad, siempre debe tratar `precompileUpgrades` como solo de agregado.

Como medida de último recurso, es posible abortar o reconfigurar una actualización de precompilación que no se ha
activado desde que la cadena todavía está procesando bloques usando el conjunto de reglas anterior.

:::

Si es necesario abortar una actualización, puede eliminar la actualización de precompilación de `upgrade.json`
desde el final de la lista de actualizaciones. Mientras la cadena de bloques no haya aceptado un bloque con un
marca de tiempo posterior a la marca de tiempo de esa actualización, abortará la actualización para ese nodo.
...
Este ejemplo habilita `feeManagerConfig` en el primer bloque con marca de tiempo >= `1668950000`, habilita
`txAllowListConfig` en el primer bloque con marca de tiempo >= `1668960000` y deshabilita `feeManagerConfig`
en el primer bloque con marca de tiempo >= `1668970000`.

Cuando una desactivación de precompilación entra en efecto (es decir, después de que su `blockTimestamp` ha pasado), su almacenamiento
se borrará. Si desea volver a habilitarlo, deberá tratarlo como una nueva configuración.

Después de haber creado `upgrade.json` y colocarlo en el directorio de configuración de la cadena, debe
reiniciar el nodo para que se cargue el archivo de actualización (nuevamente, ¡asegúrese de no reiniciar todos los validadores de la Subnet a la vez!). Al reiniciar el nodo, imprimirá la configuración de la cadena, donde
...
github.com/ava-labs/subnet-evm/eth/backend.go:155: Configuración de la cadena inicializada
...
Ten en cuenta que la entrada `precompileUpgrades` refleja correctamente los cambios. También puedes verificar
los precompiles activados en un momento dado con el método RPC [`eth_getActivePrecompilesAt`](/reference/subnet-evm/api#eth_getactiveprecompilesat).
El método RPC [`eth_getChainConfig`](/reference/subnet-evm/api#eth_getchainconfig) también devolverá las actualizaciones
configuradas en la respuesta.

¡Eso es todo, tu Subnet está lista y las actualizaciones deseadas se activarán en el momento indicado!

### Configuraciones iniciales de precompiles

Los precompiles pueden ser gestionados por algunas direcciones privilegiadas para cambiar sus configuraciones y activar
sus efectos. Por ejemplo, el precompile `feeManagerConfig` puede tener `adminAddresses` que pueden
...
la estructura de tarifas de la red. La dirección de administrador debe llamar al precompile para activar
...
la dirección `0x8db97C7cEcE249c2b98bDC0226Cc4C2A57BF52FC` es comprometida, la red está comprometida.

Con las configuraciones iniciales, los precompiles pueden activar inmediatamente su efecto en el momento de activación.
De esta manera, las direcciones de administrador pueden omitirse de la configuración de precompiles. Por ejemplo,
el precompile `feeManagerConfig` puede tener `initialFeeConfig` para configurar la configuración de tarifas en la
activación:
...
Ten en cuenta que no hay un campo `adminAddresses` en la configuración. Esto significa que no habrá
direcciones de administrador para gestionar la estructura de tarifas con este precompile. El precompile simplemente
...
Si deseas cambiar la configuración inicial del precompile, primero deberás desactivarlo y luego
activar el precompile nuevamente con la nueva configuración.

:::

Consulta cada configuración inicial de precompiles en sus secciones relevantes de `Configuración inicial` bajo [Precompiles](#precompiles).

## Configuraciones de Cadena AvalancheGo

Como se describe en [este documento](/nodes/configure/chain-config-flags.md#subnet-chain-configs), cada blockchain
de las Subnets puede tener su propia configuración personalizada. Si el ID de cadena de una Subnet es
`2ebCneCbwthjQ1rYT41nhd7M76Hc6YmosMAQrTFhBq8qeqh6tt`, el archivo de configuración para esta cadena se encuentra en
`{chain-config-dir}/2ebCneCbwthjQ1rYT41nhd7M76Hc6YmosMAQrTFhBq8qeqh6tt/config.json`.

Para blockchains creadas por o bifurcadas de Subnet-EVM, la mayoría de las
configuraciones de [C-Chain](/nodes/configure/chain-config-flags.md#c-chain-configs) son aplicables, excepto
[APIs específicas de Avalanche](/nodes/configure/chain-config-flags.md#enabling-avalanche-specific-apis).

### Regossip de prioridad

Una transacción se "regossipea" cuando el nodo no encuentra la transacción en
un bloque después de `priority-regossip-frequency` (por defecto, `1m`). Por defecto, hasta 16 transacciones
(máximo 1 por dirección) se regossipean a validadores por minuto.

Los operadores pueden usar "regossip de prioridad" para "regossipear" más agresivamente las transacciones para un conjunto de
direcciones importantes (como los relayers de puentes). Para hacerlo, deberás actualizar tu
[configuración de cadena](/nodes/configure/chain-config-flags.md#subnet-chain-configs) con lo siguiente:
...
Por defecto, hasta 32 transacciones de direcciones prioritarias (máximo 16 por dirección) se regossipean a
validadores por segundo. Puedes anular estos valores predeterminados con la siguiente configuración:
...
### Recipiente de tarifas (Fee Recipient)


Esto funciona junto con [`allowFeeRecipients`](#setting-a-custom-fee-recipient) y [RewardManager
precompile](#changing-fee-reward-mechanisms) para especificar a dónde se deben enviar las tarifas.
...
Subnet-EVM permite a los operadores de red especificar una modificación de estado que tendrá lugar
al comienzo del primer bloque con una marca de tiempo mayor o igual a la especificada
en la configuración.

Esto proporciona un camino de último recurso para actualizar contratos no actualizables a través de una actualización de red
(por ejemplo, para solucionar problemas cuando se ejecuta su propia cadena de bloques).

:::warning

Esto solo debe usarse como una alternativa de último recurso a bifurcar `subnet-evm` y especificar
la actualización de red en el código.

Usar una actualización de red para modificar el estado no es parte de las operaciones normales de la
EVM. Debe asegurarse de que las modificaciones no invaliden ninguna de las suposiciones de
los contratos desplegados o causen incompatibilidades con infraestructura aguas abajo como
exploradores de bloques.

:::

Las marcas de tiempo para las actualizaciones en `stateUpgrades` deben estar en orden creciente.
`stateUpgrades` se puede especificar junto con `precompileUpgrades` o por sí mismo.

Se admiten las siguientes tres modificaciones de estado:

- `balanceChange`: agrega una cantidad especificada al saldo de una cuenta dada. Esta cantidad puede ser
  especificada en hexadecimal o decimal y debe ser positiva.
- `storage`: modifica las ranuras de almacenamiento especificadas a los valores especificados. Las claves y valores deben
  ser de 32 bytes especificados en hexadecimal, con un prefijo `0x`.
- `code`: modifica el código almacenado en la cuenta especificada. El
  código debe ser _solo_ la parte de tiempo de ejecución de un código. El código debe comenzar con un prefijo `0x`.

:::warning

Si se modifica el código, _solo_ se debe proporcionar la parte de tiempo de ejecución del bytecode en
`upgrades.json`. No use el bytecode que se usaría para desplegar un nuevo contrato, ya que esto
incluye el código del constructor también. Consulte la documentación de su compilador para obtener información
sobre cómo encontrar la parte de tiempo de ejecución del contrato que desea modificar.

:::

El archivo `upgrades.json` que se muestra a continuación describe una actualización de red que hará las siguientes
modificaciones de estado en el primer bloque después (o en) `8 de marzo de 2023 1:30:00 AM GMT`:
