...
contract('Collectible', ([contractDeployer, creator, buyer]) => {
  ...
  describe('Mint an NFT and set a royalty.', async () => {
    ...
    it('Give a new id to a newly created token', async () => {
      const newTokenId = await collectible.createCollectible.call('metadata', 20, {
        from: creator
      });
    ...
    it('Mint a NFT and emit events.', async () => {
      ...
      assert.equal(result.logs.length, 2, 'Should trigger two events.'); //event Transfer
      ...
      assert.equal(result.logs[0].args.tokenId, 1, 'Should log the token id which is 1.'); //event ItemMinted
    ...
    it('Check if hash has been minted and that you cannot mint the same hash again.', async () => {
      ...
      await expectRevert(collectible.createCollectible('metadata', 30, {
        from: creator
