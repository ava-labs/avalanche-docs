...
- **info-api-url:** The URL of the [Info API](https://build.avax.network/docs/api-reference/info-api) node to which the relayer will connect to to receive information like the NetworkID.
- **p-chain-api-url:** The URL of the Avalanche [P-Chain API](https://build.avax.network/docs/api-reference/p-chain/api) node to which the relayer will connect to query the validator sets. 
