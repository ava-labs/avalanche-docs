...
For a full list of available APIs see [here](/docs/api-reference/quicklinks).
...
The maximum number of blocks to serve per `getLogs` request is 2048, which is set by [`api-max-blocks-per-request`](/docs/nodes/configure/chain-configs/c-chain#api-max-blocks-per-request).
