...
The prefix is simply the string `\x1AAvalanche Signed Message:\n`, where `0x1A` is the length of the prefix text and `length(message)` is an [integer](/api-reference/standards/serialization-primitives#integer) of the message size.
