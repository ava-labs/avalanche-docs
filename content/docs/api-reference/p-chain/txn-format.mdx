...
or `P-Chain`. This document uses the [primitive serialization](/api-reference/standards/serialization-primitives) format for packing and
...
A [secp256k1](/api-reference/standards/cryptographic-primitives#secp256k1-addresses) output owners
...
- **`SubnetID`** is the 32 byte Avalanche L1 ID that the validator is being removed from.
...
- **`Validator`** Validator has a `NodeID`, `StartTime`, `EndTime`, and `Weight`
  ...
  - **`EndTime`** is a long which is the Unix time when the validator stops validating.
  - **`Weight`** is a long which is the amount the validator stakes
- **`SubnetID`** is the 32 byte Avalanche L1 ID of the Avalanche L1 this validator will validate.
...
- **`Validator`** Validator has a `NodeID`, `StartTime`, `EndTime`, and `Weight`
  ...
  - **`EndTime`** is a long which is the Unix time when the validator stops validating.
  - **`Weight`** is a long which is the amount the validator stakes
- **`SubnetID`** is the 32 byte Avalanche L1 ID of the Avalanche L1 this delegation is on.
- **`StakeOuts`** An array of Transferable Outputs. Where to send staked tokens when done validating.
...
- **`SubnetAuth`** contains `SigIndices` and has a type id of `0x0000000a`.
  `SigIndices` is a list of unique ints that define the addresses signing the
  control signature to add a validator to a Avalanche L1. The array must be sorted low
  to high.
...
the owners of a Avalanche L1 approve something.

- **`TypeID`** is the ID for this type. It is `0x0000000a`.
- **`AddressIndices`** defines which addresses' signatures will be attached to
  this transaction. AddressIndices[i] is the index in a Avalanche L1 owner list that
...
Let's make a Avalanche L1 auth:
