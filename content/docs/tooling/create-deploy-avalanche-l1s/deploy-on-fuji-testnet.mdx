...
description: This tutorial shows how to deploy a Avalanche L1 on Fuji Testnet.
---

<Callout title="Note">
This document describes how to use the new Avalanche-CLI to deploy a Avalanche L1 on `Fuji`.
</Callout>

After trying out a Avalanche L1 on a local box by following [this tutorial](/avalanche-l1s/deploy-a-avalanche-l1/local-network), next step is to try it out on `Fuji` Testnet.

In this article, it's shown how to do the following on `Fuji` Testnet.

- Create a Avalanche L1.
...
As you want to create an EVM Avalanche L1, just accept the default `Subnet-EVM`. Next, CLI asks for the ChainID. You should provide your own ID. Check [chainlist.org](https://chainlist.org/) to see if the value you'd like is already in use.

```bash
✔ Subnet-EVM
creating subnet testblockchain
...
The final question is asking for precompiles. Precompiles are powerful customizations of your EVM. Read about them at [precompiles](/avalanche-l1s/upgrade/customize-avalanche-l1#precompiles).
...
+-------------+-------------+----------+---------------------------------------------------+------------+-----------+
| testblockchain  | testblockchain  |     3333 | tGBrM2SXkAdNsqzb3SaFZZWMNdzjjFEUKteheTa4dhUwnfQyu | Subnet-EVM | false     |
+-------------+-------------+----------+---------------------------------------------------+------------+-----------+
```
...
+-------------+-------------+---------------------------------------------------+---------------+-----------------------------------------------------------------+---------+
| testblockchain  | testblockchain  | tGBrM2SXkAdNsqzb3SaFZZWMNdzjjFEUKteheTa4dhUwnfQyu | Yes           | SubnetID: XTK7AM2Pw5A4cCtQ3rTugqbeLCU9mVixML3YwwLYUJ4WXN2Kt     | No      |
+             +             +                                                   +               +-----------------------------------------------------------------+---------+
...
Avalanche L1s are currently permissioned only. Therefore, the process now requires the user to provide _which keys can control the Subnet_. CLI prompts the user to provide one or more **P-Chain addresses**. Only the keys corresponding to these addresses are going to be able to add or remove validators. Make sure to provide **Fuji P-Chain** addresses -`P-Fuji....`\-.
...
To get your new Avalanche L1 information, visit the [Avalanche L1 Explorer](https://subnets-test.avax.network/). The search best works by blockchain ID, so in this example, enter `2XDnKyAEr1RhhWpTpMXqrjeejN23vETmDykVzkb4PrU1fQjewh` into the search box and you should see your shiny new blockchain information.

Request to Join a Avalanche L1 as a Validator[​](#request-to-join-a-avalanche-l1-as-a-validator "Direct link to heading")
-------------------------------------------------------------------------------------------------------------

The new Avalanche L1 created in the previous steps doesn't have any dedicated validators yet. To request permission to validate a Avalanche L1, the following steps are required:

<Callout title="Note">
Before a node can be a validator on a Avalanche L1, the node is required to already be a validator on the primary network, which means that your node has **fully bootstrapped**.
...
If choosing "Manual" instead, the tool is going to just print _instructions_. The user is going to have to follow these instructions and apply them to the node. Note that the IDs for the VM and Avalanche L1s is going to be different in your case.
...
Now that the node has joined the Avalanche L1, a Avalanche L1 control key holder must call `addValidator` to grant the node permission to be a validator in your Avalanche L1.
