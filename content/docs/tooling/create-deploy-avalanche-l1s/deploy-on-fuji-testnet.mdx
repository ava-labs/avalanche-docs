...
2. Move AVAX from the C-Chain to the P-Chain:
```bash
avalanche key transfer --fuji --c-chain-sender --p-chain-receiver --key mytestkey --amount 2
```
Here’s what the flags mean:
	•	`--fuji`: Perform this transfer on Fuji Testnet.
	•	`--c-chain-sender`: The source of the AVAX (C-Chain).
	•	`--p-chain-receiver`: The destination (P-Chain).
	•	`--key mytestkey`: Use the private key named “mytestkey”.
	•	`--amount 2`: Transfer 2 AVAX.
Your C-Chain balance decreases by 2 AVAX (+fee), and your P-Chain balance increases by 2 AVAX.
