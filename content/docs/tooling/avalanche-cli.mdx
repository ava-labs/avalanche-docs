---
title: Introduction
description: CLI is a command-line tool that gives developers access to everything Avalanche.
---
...
--config string             config file (default is $HOME/.cli/config.json)
...
Customize configuration for CLI
...
- [`migrate`](#migrate): migrate command migrates old ~/.cli.json and ~/.cli/config to /.cli/config.json..
...
migrate command migrates old ~/.cli.json and ~/.cli/config to /.cli/config.json..
