...
- Created a Custom VM, as described [here](/virtual-machines).
...
Let's create a Subnet called `<blockchainName>`, with custom VM binary and genesis.
...
```bash
avalanche blockchain configure subnetName
```
...
```bash
avalanche blockchain upgrade import subnetName
```
...
You can also use `avalanche node update subnet <blockchainName>` to reinstall the binary when the branch is updated, or update the config files.