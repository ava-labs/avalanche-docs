---
title: Validate a Avalanche L1
description: This page demonstrates how to configure nodes to validate an Avalanche Avalanche L1.
---

This page demonstrates how to configure nodes to validate an Avalanche Avalanche L1. Avalanche L1 Validation via Avalanche-CLI is currently only supported on Fuji and Devnet.
...
- Deployed a Avalanche L1 on [Fuji](/avalanche-l1s/deploy-a-avalanche-l1/fuji-testnet) or Devnet
...
To have all nodes in cluster `clusterName` sync with Avalanche L1 `subnetName`, run:

```bash
avalanche node sync <clusterName> <blockchainName>
```

All the nodes in cluster `clusterName` will now be syncing to Avalanche L1 `subnetName`.
...
Be a Avalanche L1 Validator[​](#be-a-avalanche-l1-validator "Direct link to heading")
-------------------------------------------------------------------------
...
We will then proceed with adding the node as a Avalanche L1 Validator. Similar to adding the node as a Primary Network Validator, the wizard will ask you for how you would like to pay for the transaction fee.
...
Once all input is completed, you will another transaction ID indicating that the node is now scheduled to be a Avalanche L1 Validator.

Once the start time that you chose for the node to be a Avalanche L1 Validator has elapsed, verify that the node is a Avalanche L1 validator by running `avalanche node status <clusterName> --subnet <blockchainName>`.
