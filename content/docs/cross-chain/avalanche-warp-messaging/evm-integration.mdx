...
The `blockchainID` in Avalanche refers to the txID that created the blockchain on the Avalanche P-Chain ([docs](/api-reference/p-chain/txn-format#unsigned-create-chain-tx)).
...
To support C-Chain to Avalanche L1 communication, or more generally Primary Network to Avalanche L1 communication, we special case the C-Chain for two reasons:
