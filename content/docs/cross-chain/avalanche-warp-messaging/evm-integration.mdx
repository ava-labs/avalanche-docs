...
For more details on Avalanche Warp Messaging, see the AvalancheGo [Warp README](https://build.avax.network/docs/build/cross-chain/awm/deep-dive).
...
The `blockchainID` in Avalanche refers to the txID that created the blockchain on the Avalanche P-Chain ([docs](https://build.avax.network/docs/specs/platform-transaction-serialization#unsigned-create-chain-tx)).
