...
description: Learn how to deploy a Avalanche L1 on the Fuji Testnet.
---
...
```bash
avalanche blockchain create testAvalanche L1
```
...
```bash
✔ Subnet-EVM
creating Avalanche L1 testblockchain
...
Avalanche L1s are currently permissioned only. Therefore, the process now requires the user to provide _which keys can control the Avalanche L1_. CLI prompts the user to provide one or more **P-Chain addresses**. Only the keys corresponding to these addresses are going to be able to add or remove validators. Make sure to provide **Fuji P-Chain** addresses -`P-Fuji....`\-.
...
```bash
avalanche blockchain export test Avalanche L1
