...
description: Learn how to create a Avalanche L1 with a custom virtual machine and deploy it locally.
---

This tutorial walks through the process of creating a Avalanche L1 with a custom virtual machine and deploying it locally. Although the tutorial uses a fork of Subnet-EVM as an example, you can extend its lessons to support any custom VM binary.
...
Open a text editor and copy the preceding text into a file called `custom_genesis.json`. For full breakdown of the genesis file, see the [Genesis File](avalanche-l1s/upgrade/customize-avalanche-l1.mdx#genesis).
