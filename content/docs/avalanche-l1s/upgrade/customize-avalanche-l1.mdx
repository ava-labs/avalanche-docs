---
title: Customize a Avalanche L1
...
A Avalanche L1 can have one or more blockchains. For example, the Primary Network, which is a Avalanche L1, a special one nonetheless, has 3 blockchains. Each chain can be further customized using chain specific configuration file. See [here](/nodes/configure/configs-flags) for detailed explanation.
...
A Avalanche L1 can customized by setting parameters for the following:

- [Validator-only communication to create a private Avalanche L1](/nodes/configure/avalanche-l1-configs#validatoronly-bool)
- [Consensus](/nodes/configure/avalanche-l1-configs#consensus-parameters)
- [Gossip](/nodes/configure/avalanche-l1-configs#gossip-configs)

See [here](/nodes/configure/avalanche-l1-configs) for more info.
...
Warp Precompile enabled cross-avalanche-l1 communication between other Avalanche L1s and primary-network (C-Chain). In order to use Warp messaging, Subnet-EVM chains must activate their Warp precompiles. Warp can be activated with the following lines in upgrade.json:
...
As described in [this doc](/nodes/configure/configs-flags#avalanche-l1-chain-configs), each blockchain of Avalanche L1s can have its own custom configuration. If a Avalanche L1's ChainID is `2ebCneCbwthjQ1rYT41nhd7M76Hc6YmosMAQrTFhBq8qeqh6tt`, the config file for this chain is located at `{chain-config-dir}/2ebCneCbwthjQ1rYT41nhd7M76Hc6YmosMAQrTFhBq8qeqh6tt/config.json`.
