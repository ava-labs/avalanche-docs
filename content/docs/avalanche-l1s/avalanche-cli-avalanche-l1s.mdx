...
The `subnet` command suite provides a collection of tools for developing and deploying Avalanche L1s.
...
To add the validator to the Avalanche L1's allow list, you first need to provide the subnetName and the validator's unique NodeID. The command then prompts for the validation start time, duration, and stake weight. You can bypass these prompts by providing the values with flags.
...
To get the bootstrap status of a node with a Avalanche L1, use the `--subnet` flag.
...
The `node sync` command enables all nodes in a cluster to be bootstrapped to a Avalanche L1. You can check the Avalanche L1 bootstrap status by calling avalanche `node status <clusterName> --subnet <blockchainName>`
