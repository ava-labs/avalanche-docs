...
- To use a third-party service to host your node or run a validator, [see here](/docs/nodes/on-third-party-services/amazon-web-services) for dedicated tutorials.  
- If you're just interested in setting up a node for staking, it's recommended to follow the [AvalancheGo Install Script](/docs/nodes/using-install-script/installing-avalanche-go) tutorial.
</Accordion>
</Accordions>
...
For more information on the requests available at these endpoints, please see the [AvalancheGo API Reference](/api-reference/p-chain/api) documentation.
