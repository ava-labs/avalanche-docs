...
To run our e2e contract tests, we will need to create a Avalanche L1 that has the `Hello World` precompile activated, so we will copy and paste the below genesis file into: `/tests/precompile/genesis/hello_world.json`.
...
You should have cloned [AvalancheGo](https://github.com/ava-labs/avalanchego) within your `$GOPATH` in the [Background and Requirements](/virtual-machines/evm-customization/background-requirements) section, so you can build AvalancheGo with the following command:
