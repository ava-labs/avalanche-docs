...
Avalanche's Primary Network is an Avalanche L1 that has three chains: P-Chain, X-Chain, and C-Chain. The C-Chain is an instance of the Ethereum Virtual Machine powered by Avalanche's Snowman consensus protocol. The [C-Chain RPC](/api-reference/c-chain/api) can do anything a typical Ethereum client can by using the Ethereum-standard RPC calls.
...
On a local network, you can easily fund your addresses by following [this](/docs/avalanche-l1s/build-first-avalanche-l1#importing-the-test-private-key) guide.
