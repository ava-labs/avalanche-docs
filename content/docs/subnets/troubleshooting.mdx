...
2.  Use Avalanche-CLI to deploy an older version of AvalancheGo by using the `--avalanchego-version` flag. Both the [`subnet deploy`](/docs/tooling/avalanche-cli#subnet-deploy) and [`network start`](/docs/tooling/avalanche-cli#network-start) commands support setting the AvalancheGo version explicitly.
