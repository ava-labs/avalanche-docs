...
This tutorial specifically covers tracking a Subnet built with Avalanche's [Subnet-EVM](https://github.com/ava-labs/subnet-evm), the default [Virtual Machine](/docs/learn/virtual-machines) run by Subnets on Avalanche.

Build AvalancheGo[​](#build-avalanchego "Direct link to heading")
-----------------------------------------------------------------

It is recommended that you first complete [this comprehensive guide](/docs/nodes/run-a-node/manually) which demonstrates how to build and run a basic Avalanche node. Below are the high level details.
...
It may take a few hours for the node to fully [bootstrap](/docs/nodes/run-a-node/manually#bootstrapping) to the Avalanche Primary Network and tracked Subnets.
