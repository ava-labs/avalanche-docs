...
<BlockMath math="\text{Fee Rate} = M \cdot \exp\left(\frac{x}{K}\right)" />
...
value simply K controls how quickly the price changes, and x is updated every second based on the difference between the number of active validators
V and the target T:

<BlockMath math="x = \max(x + V - T, 0)" />
