...
This file is meant to be the single source of truth for how we serialize transactions in the Avalanche Virtual Machine \(AVM\). This document uses the [primitive serialization](untitled-1.md) format for packing and [secp256k1](untitled-3.md#cryptography-in-the-avalanche-virtual-machine) for cryptographic user identification.
...
A transferable output contains an `AssetID` and an [`Output`](untitled.md#outputs).

* **`AssetID`** is a 32-byte array that defines which asset this output references.
* **`Output`** is an output, as defined [below](untitled.md#secp-256-k1-transfer-output). For example, this can be a [SECP256K1 transfer output](untitled.md#secp-256-k1-transfer-output).
...
* **`Input`** is an input, as defined below. This can currently only be a [SECP256K1 transfer input](untitled.md#secp-256-k1-transfer-input)
...
* **`TransferOp`** is a [transferable operation object](untitled.md#operations).
...
Outputs have four possible types: [`SECP256K1TransferOutput`](untitled.md#secp-256-k1-transfer-output), [`SECP256K1MintOutput`](untitled.md#secp-256-k1-mint-output), [`NFTTransferOutput`](untitled.md#nft-transfer-output) and [`NFTMintOutput`](untitled.md#nft-mint-output).

## SECP256K1 Transfer Output

A [secp256k1](untitled-3.md#secp-256-k1-addresses) transfer output allows for sending a quantity of an asset to a collection of addresses after a specified unix time.
...
A [secp256k1](untitled-3.md#secp-256-k1-addresses) mint output is an output that is owned by a collection of addresses.
...
A [secp256k1](untitled-3.md#secp-256-k1-addresses) transfer input allows for spending an unspent secp256k1 transfer output.
...
A [secp256k1](untitled-3.md#secp-256-k1-addresses) mint operation consumes a SECP256K1 mint output, creates a new mint output and sends a transfer output to a new set of owners.
...
* **`AddressIndices`** is a list of unique ints that define the private keys that are being used to spend the [UTXO](untitled.md#utxo). Each UTXO has an array of addresses that can spend the UTXO. Each int represents the index in this address array that will sign this transaction. The array must be sorted low to high.
* **`MintOutput`** is a [SECP256K1 Mint output](untitled.md#secp-256-k1-mint-output-example).
* **`TransferOutput`** is a [SECP256K1 Transfer output](untitled.md#secp-256-k1-transfer-output).
...
Let’s make a [secp256k1](untitled-3.md#secp-256-k1-addresses) mint operation with:
...
* **`AddressIndices`** is a list of unique ints that define the private keys that are being used to spend the UTXO. Each UTXO has an array of addresses that can spend the UTXO. Each int represents the index in this address array that will sign this transaction. The array must be sorted low to high.
* **`NFTTransferOutput`** is the output of this operation and must be an [NFT Transfer Output](untitled.md#nft-transfer-output). This output doesn’t have the **`TypeId`**, because the type is known by the context of being in this operation.
...
* **`Outputs`** is a variable length array of [outputs](untitled.md#outputs), as defined above.
...
A [secp256k1](untitled-3.md#secp-256-k1-addresses) credential contains a list of 65-byte recoverable signatures.
...
An NFT credential is the same as an [secp256k1 credential](untitled.md#secp-256-k1-credential) with a different TypeID. The TypeID for an NFT credential is `0x0000000e`.

## Unsigned Transactions

Unsigned transactions contain the full content of a transaction with only the signatures missing. Unsigned transactions have four possible types: [`CreateAssetTx`](untitled.md#what-unsigned-create-asset-tx-contains), [`OperationTx`](https://app.gitbook.com/@nicolas-avalabs/s/avalanche-documentation/~/drafts/-MW-wWlPGhWIPdrdd9pJ/materiel-de-reference/untitled#what-unsigned-create-asset-tx-contains), [`ImportTx`](https://app.gitbook.com/@nicolas-avalabs/s/avalanche-documentation/~/drafts/-MW-wWlPGhWIPdrdd9pJ/materiel-de-reference/untitled#what-unsigned-create-asset-tx-contains), and [`ExportTx`](https://app.gitbook.com/@nicolas-avalabs/s/avalanche-documentation/~/drafts/-MW-wWlPGhWIPdrdd9pJ/materiel-de-reference/untitled#what-unsigned-create-asset-tx-contains). They all embed [`BaseTx`](https://app.gitbook.com/@nicolas-avalabs/s/avalanche-documentation/~/drafts/-MW-wWlPGhWIPdrdd9pJ/materiel-de-reference/untitled#what-unsigned-create-asset-tx-contains), which contains common fields and operations.
...
* **`InitialStates`** is a variable length array that defines the feature extensions this asset supports, and the [initial state](untitled.md#initial-state) of those feature extensions.
...
* **`BaseTx`**
* **`Ops`** is a variable-length array of [Transferable Ops.](untitled.md#transferable-op)
...
* **`Ins`** is a variable length array of [Transferable Inputs](https://app.gitbook.com/@nicolas-avalabs/s/avalanche-documentation/~/drafts/-MW-wWlPGhWIPdrdd9pJ/materiel-de-reference/untitled#transferable-input).
...
* **`Outs`** is a variable length array of [Transferable Outputs](untitled.md#transferable-output).
...
A signed transaction is an unsigned transaction with the addition of an array of [credentials](untitled.md#credentials).
...
* **`Credentials`** is an array of [credentials](untitled.md#credentials). Each credential will be paired with the input in the same index at this credential.
...
* **`Output`** is the [output](untitled.md#outputs) object that created this utxo. The serialization of Outputs was defined above.
