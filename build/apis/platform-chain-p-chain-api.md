...
This API allows clients to interact with the [P-Chain](../../learn/platform-overview/#platform-chain-p-chain), which maintains Avalanche’s validator set and handles blockchain creation.
...
Note that once you issue the transaction to add a node as a delegator, there is no way to change the parameters. **You can’t remove a stake early or change the stake amount, node ID, or reward address.** Please make sure you’re using the correct values. If you’re not sure, ask for help on [Discord.](https://chat.avalabs.org/)
...
Note that once you issue the transaction to add a node as a validator, there is no way to change the parameters. **You can’t remove stake early or change the stake amount, node ID, or reward address.** Please make sure you’re using the correct values. If you’re not sure, ask for help on [Discord.](https://chat.avalabs.org/)
...
Send AVAX from an address on the P-Chain to an address on the X-Chain. After issuing this transaction, you must call the X-Chain’s [`importAVAX`](https://docs.avax.network/v1.0/en/api/avm/#avmimportavax) method to complete the transfer.
...
See [here](https://docs.avax.network/v1.0/en/tutorials/adding-validators/#add-a-validator-to-a-subnet) for information on adding a validator to a Subnet.
...
Before this method is called, you must call the X-Chain’s [`exportAVAX`](https://docs.avax.network/v1.0/en/api/avm/#avmexportavax) method to initiate the transfer.
