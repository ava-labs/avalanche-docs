...
エンコーディングは、任意のバイト、つまり返されるジェネシスバイトに使用するエンコードフォーマットを指定します。「cv58」あるいは「hex」のいずれかを指定します。。デフォルトは「cb58」です。
...
X-ChainからP-ChainまたはC-Chainに資産を送信します。このメソッドを呼び出した後、[C-Chainの`avax.import`](contract-chain-c-chain-api.md#avax-import)あるいは[P-Chainの`avax.importAVAX`](platform-chain-p-chain-api.md#avax-importAVAX)を呼び出し、転送を完了させる必要があります。
...
* `assetID`は、送信された資産の資産IDです。AVAXのエクスポートには`AVAX`を使用します。
* `from`は、この操作に使用したいアドレスです。省略した場合は、必要に応じて自分のアドレスのいずれかを使用します。
* `changeAddr`は、変更が行われた場合に送信されるアドレスです。省略した場合は、ユーザーが管理するいずれかのアドレスに送信されます。
...
指定されたアドレスを管理する秘密鍵を取得します。返された秘密鍵は、[`avm.importKey`](exchange-chain-x-chain-api.md#avm-importkey)で、ユーザーに追加できます。
...
* `credentials`は、このトランザクションの認証情報のリストです。各認証情報により、このトランザクションの作成者がこのトランザクションの入力のひとつを消費する許可を得ていることを証明しています。各認証情報は、署名のリストです。
* `unsignedTx`は、トランザクションの非署名の部分です。
* `networkID`はこのトランザクションが発生したネットワークのIDです。（Avalanche Mainnetは`1`です。）
* `blockchainID`はこのトランザクションが発生したブロックチェーンのIDです。（Avalanche MainnetのX-Chainは`2oYMBNV4eNHyqk2fjjV5nVQLDbtmNJzq5s3qs3Lo6ftnC6FByM`です。）
* `outputs`の各要素は、別のチェーンにエクスポートされないこのトランザクションの出力（UTXO）です。
* の各要素は、他のチェーンからインポートされないこのトランザクションの入力`inputs`です。
* インポートトランザクションには、追加のフィールド`sourceChain`と`importedInputs`があり、それにより資産のインポート元のブロックチェーンIDとインポートされる入力を指定します。
* エクスポートトランザクションには追加のフィールド`destinationChain`と`exportedOutputs`があり、それにより資産をエクスポートする先のブロックチェーンIDとエクスポートされるUTXOをを指定します。

出力には次のものが含まれます。

* `assetID`：転送する資産のID。（MainnetのAvaxIDは`FvwEAhmxKfeiG8SnEvq42hc6whRyY3EFYAvebMqDNDGCgxN5Z`です。）
* `fxID`：この出力が使用するFXのID。
* `output`：この出力のFX固有のコンテンツ。

ほとんどの出力は、このようにsecp256k1 FXを使用します。
...
上記の出力は、`addresses`のアドレスの`threshold`からの署名を持つトランザクションによって、Unixタイム`locktime`後に消費することができます。
...
P-ChainまたはC-ChainからX-Chainへの資産の転送を確定します。このメソッドが呼び出される前に、P-Chainの[`platform.exportAVAX`](platform-chain-p-chain-api.md#platform-exportavax)あるいはC-Chainの[`avax.export`](contract-chain-c-chain-api.md#avax-export)メソッドを呼び出して転送を開始する必要があります。
...
署名済みのトランザクションをネットワークに送信し、トランザクションが受け入れられると仮定します。`encoding`では、署名付きトランザクションのフォーマットを指定します。「cv58」あるいは「hex」のいずれかを指定します。。デフォルトは「cb58」です。
...
* **NewBloom**[のブルームフィルタリング](https://en.wikipedia.org/wiki/Bloom_filter)では、誤検知の可能性がありますが、より多くのアドレスをフィルタリングすることができます。そのアドレスがフィルターに入っていれば、トランザクションが送信されます。
