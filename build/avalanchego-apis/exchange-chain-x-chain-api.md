...
[X-Chain](../../learn/platform-overview/#exchange-chain-x-chain)は、 Avalanche Virtual Machine \(AVM\)のインスタンスで、資産を作成、取引するためのAvalancheのネイティブプラットフォームです。このAPIにより、クライアントはX-ChainやAVMの他のインスタンス上で資産を作成、取引することができます。

{% embed url="https://www.youtube.com/watch?v=rD-IOd1nvFo" caption="" %}

## フォーマット

このAPIは`json 2.0`RPCフォーマットを採用しています。JSONのRPC呼び出しの詳細については、[こちら](issuing-api-calls.md)をご覧ください。
...
エンコーディングは、任意のバイトに使用するエンコードフォーマットを指定します。つまり、返ってくるジェネシスバイトです。「cb58」または「16進数」のいずれかを指定します。デフォルトは「cb58」です。
...
新たな固定キャップの代替性資産を作成します。初期化時に数量が作成され、その後は作成されません。この資産は`avm.send`で送ることができます。
...
* `name`は、その資産の人間が読める名前です。必ずしも一意である必要はありません。
* `symbol`は、資産を表す略記号です。0～4文字で表示されます。必ずしも一意である必要はありません。省略可能です。
* `denomination`は、この資産の残高をユーザーインターフェースでどのように表示するかを決定します。`denomination`が0の場合、この資産の100単位は100と表示されます。`denomination`が1の場合、この資産の100単位は10.0と表示されます。`denomination`が2の場合、この資産の100単位は1.00と表示される、などです。デフォルトは0です。
* `from`は、この操作に使用するアドレスです。省略した場合は、必要に応じて任意のアドレスを使用します。
* `changeAddr`は、変更が送信されるアドレスです。省略した場合は、ユーザーが管理しているアドレスのいずれかに送信されます。
...
* `from`は、この操作に使用するアドレスです。省略した場合は、必要に応じて任意のアドレスを使用します。
* `changeAddr`は、変更が送信されるアドレスです。省略した場合は、ユーザーが管理しているアドレスのいずれかに送信されます。
* `username`は、トランザクション手数料を支払うユーザーです。`username`は、この資産をさらにミントすることを許可する鍵を保持していなければなりません。つまり，ミンターセットの1つについて，少なくとも_閾値_の鍵を管理していなければなりません。
...
新しい可変キャップ、すなわち代替性資産を作成します。初期化時には、この資産の単位は存在しません。ミンターは、この資産の単位をを使って`avm.mint`ミントすることができます。
...
* `name`は、その資産の人間が読める名前です。必ずしも一意である必要はありません。
* `symbol`は、資産を表す略記号です。0～4文字で表示されます。必ずしも一意である必要はありません。省略可能です。
* `denomination`は、この資産の残高をユーザーインターフェースでどのように表示するかを決定します。デノミネーションが0の場合、この資産の100単位は100と表示されます。デノミネーションが1の場合、この資産の100単位は10.0と表示されます。デノミネーションが2の場合、この資産の100単位は.100などと表示されます。
...
新しい非代替性資産を作成します。初期化時には、この資産の単位は存在しません。ミンターはを使`avm.mintNFT`用してこの資産の単位をミントすることができます。
...
* `payload`は、最大1024バイトの任意のペイロードです。そのエンコードフォーマットは、`encoding`引数で指定します。
* `from`は、この操作に使用するアドレスです。省略した場合は、必要に応じて任意のアドレスを使用します。
* `changeAddr`は、変更が送信されるアドレスです。省略した場合は、ユーザーが管理しているアドレスのいずれかに送信されます。
* `username`は、トランザクション手数料を支払うユーザーです。`username`は、この資産をさらにミントすることを許可する鍵を保持していなければなりません。つまり，ミンターセットの1つについて，少なくとも_閾値_の鍵を管理していなければなりません。
* `txID`は、このトランザクションのIDです。
* `changeAddr`の結果には、変更があった場合の送付先が表示されます。
* `encoding`は、ペイロード引数に使用するエンコーディングフォーマットです。「cb58」または「16進数」のいずれかです。デフォルトは「cb58」です。
...
X-Chainから他のチェーンにAVAXを送信します。このメソッドを呼び出した後、他のチェーン上の`import`を呼び出して転送を完了させる必要があります。
...
指定されたアドレス  を管理する秘密鍵を取得します。
返ってくる秘密鍵は、[`avm.importKey`](exchange-chain-x-chain-api.md#avm-importkey)でユーザーに追加することができます。
...
指定したアドレスの残高を変更したすべてのトランザクションを返します。どちらかがTrueであれば、次のトランザクションはアドレスの残高を変更します。
...
* `assetID`：この資産の残高を変更したトランザクションのみを返します。資産のIDまたはエイリアスでなければなりません。
* `pageSize`：1ページあたりに返すアイテムの数。オプションです。デフォルトは1024です。

**レスポンスパラメータ**

* `txIDs`：このアドレスの残高に影響を与えたトランザクションIDのリストです。
* `cursor`：ページ番号またはオフセット。次のページを取得するためのリクエストで使用します。
...
指定したトランザクションを返します。`encoding`パラメータは、返すトランザクションのフォーマットを設定します。`"cb58"`、`"hex"`または`"json"`を指定します。デフォルトは「cb58」です。
...
* `Accepted`：すべてのノードがトランザクションを受け入れます（または受け入れる予定です）
...
* `utxos`は、`addresses`の少なくとも1つのアドレスを各UTXOなどを参照するUTXOのリストです。
* 最大で`limit`UTXOが返ります。`limit`が省略の場合や1024より大きい場合は、1024に設定します。
* このメソッドはページネーションに対応しています。`endIndex`は、最後に返るUTXOを示します。次のセットのUTXOを取得するには、次の呼び出しで`endIndex`の値を`startIndex`として使用します。
* `startIndex`が省略の場合は、`limit`までのUTXOをすべて取得します。
* ページネーションを使用する場合（`startIndex`が提供されている場合）、UTXOは複数回の呼び出しで一意となることは保証されません。つまり、あるUTXOが最初の呼び出しの結果に現れ、その後、2回目の呼び出しで再び現れることがあります。
* ページネーションを使用する場合、複数回の呼び出しでの一貫性は保証されません。つまり、アドレスのUTXOセットが呼び出しの間に変更されている可能性があります。
* `encoding`は、返ってくるUTXOのフォーマットを設定します。「cb58」または 「16進数」のいずれかです。デフォルトは「cb58」です。
...
`numFetched`は`limit`と同じなので、取得しなかったUTXOがさらにあるのではないかと考えられます。今度は`startIndex`でこのメソッドを再度呼び出します。
...
`numFetched`が`limit`よりも小さいので、UTXOの取得が完了したことがわかります。ですから、このメソッドを再度呼び出す必要はありません。

ImportTxを構築するために、P-ChainからX-ChainにエクスポートしたUTXOを取得したいとします。この場合、アトミックUTXOを取得するためには、sourceChain引数でGetUTXOを呼び出す必要があります。
...
P-ChainまたはC-ChainからX-ChainへのAVAXの転送を確定します。このメソッドが呼び出される前に、P-Chainの[`platform.exportAVAX`](platform-chain-p-chain-api.md#platform-exportavax)またはC-Chainの[`avax.export`](contract-chain-c-chain-api.md#avax-export)メソッドを呼び出し転送を開始する必要があります。
...
* `to`は、AVAXが送られるアドレスです。これは、P-Chainの`exportAVAX`またはC-Chainの`export`の対応する呼び出しの`to`の引数と同じでなければなりません。
* `sourceChain`は、AVAXをインポートするチェーンのIDまたはエイリアスです。C-Chain,から資金をインポートする場合は`"C"`を使用します。
...
P-ChainからX-ChainへのAVAXの転送を確定します。このメソッドが呼び出される前に、P-Chainの[`platform.exportAVAX`](platform-chain-p-chain-api.md#platform-exportavax)メソッドを呼び出して転送を開始する必要があります。
...
* `to`は、AVAXが送られるアドレスです。これは、P-Chainの`exportAVAX`に対応する呼び出しの`to`の引数と同じでなければなりません。
* `sourceChain`は、AVAXをインポートするチェーンのIDまたはエイリアスです。P-Chain,から資金をインポートする場合は、`"P"`を使用します。
* `username`は、`to`を管理するユーザーです。
...
* `assetID`のIDを持つ`amount`単位の資産をのア`to`ドレスに送信します。は、`amount`その資産の最小単位で表示されます。AVAXの場合は1nAVAX（1AVAXの10億分の1）です。
* `to`は、資産の送信先であるX-Chainアドレスです。
* `from`は、この操作に使用するアドレスです。省略した場合は、必要に応じて任意のアドレスを使用します。
* `changeAddr`は、変更が送信されるアドレスです。省略した場合は、ユーザーが管理しているアドレスのいずれかに送信されます。
* `memo`を添付することができ、その長さは最大256バイトまで可能です。
* ユーザー`username`が管理するアドレスから資産を送信します。\(もちろん、そのユーザーは送信する資産の残高以上を保有している必要があります。\)
...
* `groupID`は、NFT送信元のNFTグループです。NFTの作成では、1つのNFT IDの下に複数のグループを作成することができます。各グループに対して複数のNFTを発行することができます。
* `to`は、NFTの送信先であるX-Chainアドレスです。
* `from`には、この操作に使用するアドレスを入力します。省略した場合は、必要に応じて任意のアドレスを使用します。`changeAddr`は、変更内容を送信するアドレスです。省略した場合は、ユーザーが管理しているアドレスのいずれかに変更を送信します。
* 資産は、ユーザー`username`が管理するアドレスから送信されます。（もちろん、そのユーザーは少なくとも送信するNFTの残高を保有している必要があります。）
...
ネットワークに署名付きトランザクションを送信し、そのトランザクションが受け入れられると仮定します。`encoding`は、署名付きトランザクションのフォーマットを指定します。「cb58」または「16進数」のいずれかを指定できます。デフォルトは「cb58」です。
...
**NewSet**または**NewBloom**を呼び出すとフィルターがリセットされるため、続いて**AddAddresses**を呼び出す必要があります。**AddAddresses**は複数回呼び出すことができます。

**セット内容**

* **NewSet**は絶対的なアドレス照合を行い、アドレスがアドレスセットに含まれていれば、トランザクションを行います。
* **NewBloom**の[ブルームフィルタリング](https://en.wikipedia.org/wiki/Bloom_filter)では、誤検知の可能性がありますが、より多くのアドレスをフィルタリングすることができます。 そのアドレスがフィルターに入っていれば、トランザクションを行います。
