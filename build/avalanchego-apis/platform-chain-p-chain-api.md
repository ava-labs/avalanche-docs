...
デリゲーターは、AVAXをステークし、代わって検証を行うバリデーター（デリゲーティ）を指定します。デリゲーティは、デリゲートされたステークに比例して、他のバリデーターからサンプリングされる確率（重量）が高くなります。
...
ノードをデリゲーターとして追加するトランザクションを一度発行すると、パラメータを変更することはできないことに注意してください。**ステークの早期削除や、ステーク量、ノードID、報酬アドレスの変更はできません。**正しい値を使用しているか確認してください。よくわからない場合は、[デベロッパー用FAQ](https://support.avalabs.org/en/collections/2618154-developer-faq)を確認するか、[Discord](https://chat.avalabs.org/)でヘルプを求めてください。
...
一次ネットワークにバリデーターを追加します。これを行うにはAVAXをステークする必要があります。検証中にノードが十分に正しく反応していれば、ステーク期間が終了したときに報酬を受け取ることができます。コンセンサスの際に他のバリデーターからサンプリングされる確率は、ステーク下AVAXの量に比例します。

バリデーターはデリゲーターに手数料を請求します。デリゲーターは、デリゲーターの検証報酬（ある場合）の一定の割合を受け取ります。最小デリゲート手数料は2％です。バリデーターを追加するトランザクションは無料です。

検証期間は2週間から1年の間でなければなりません。

バリデーターに課せられる総重量には上限があります。つまり、どのバリデーターにもこの値以上のAVAXがステークされたりデリゲートされたりすることはないということです。この値は初期状態では`min(5 * amount staked, 3M AVAX)`に設定されています。バリデーターの総価値は300万AVAXです。

ノードをバリデーターとして追加するトランザクションを一度発行すると、パラメータを変更することはできないことに注意してください。**早期にステークを削除したり、ステーク量、ノードID、報酬アドレスを変更することはできません。**正しい値を使用しているか確認してください。よくわからない場合は、[デベロッパー用FAQ](https://support.avalabs.org/en/collections/2618154-developer-faq)を確認するか、[Discord](https://chat.avalabs.org/)でヘルプを求めてください。
...
* `delegationFeeRate`は、他の人がこのバリデーターにステークをデリゲートしたときに、このバリデーターが請求するパーセント手数料です。小数点以下4桁まで許容され、それ以上は無視されます。0から100の間でなければなりません。例えば、`delegationFeeRate`が`1.2345`で、誰かがこのバリデーターにデリゲートした場合、デリゲート期間が終了すると、報酬の1.2345%がバリデーターに、残りがデリゲーターに支払われます。
* `from`は、この操作に使用するアドレスです。省略した場合は、必要に応じて任意のアドレスを使用します。
* `changeAddr`は、変更が行われた場合に送信されるアドレスです。省略した場合は、ユーザーが管理するいずれかのアドレスに送信されます。
* `username`は、トランザクション手数料を支払うユーザーです。
* `password`は`username`のパスワードです。
...
この例では、シェルコマンド`date`を使って、10分後と2日後のUNIX時間を計算しています。（注：Macをお使いの方は`$(date`を`$(gdate`に置き換えてください。また、`gdate`がインストールされていない場合は、`brew install coreutils`を実行してください。）
...
一次ネットワーク以外のサブネットにバリデーターを追加します。バリデーターは、このサブネットを検証している間、ずっと一次ネットワークを検証しなければなりません。
...
新しいブロックチェーンを作成します。現在は、AVMとタイムスタンプVMの新しいインスタンスの作成のみをサポートしています。　
...
* `subnetID`は、新しいブロックチェーンを検証するサブネットのIDです。サブネットは存在しなければならず、一次ネットワークにはなれません。
...
* `encoding`では、`genesisData`で使用するフォーマットを指定します。「cb58」または「16進数」のいずれかです。デフォルトは「cb58」です。仮想マシンは、`genesisData`を生成するために使用できる`buildGenesis`という名前の静的なAPIメソッドを持つ必要があります
* `from`は、この操作に使用するアドレスです。省略した場合は、必要に応じて任意のアドレスを使用します。
* `changeAddr`は、変更が行われた場合に送信されるアドレスです。省略した場合は、ユーザーが管理するいずれかのアドレスに送信されます。
* `username`は、トランザクション手数料を支払うユーザーです。このユーザーは、十分な数のサブネットのコントロールキーを持っていなければなりません。
* `password`は`username`のパスワードです。
* `txID`はトランザクションIDです。
...
P-Chain上のアドレスからX-Chain上のアドレスにAVAXを送信します。このトランザクションを発行した後、X-Chainの[`avm.importAVAX`](exchange-chain-x-chain-api.md#avm-importavax)メソッドを呼び出して転送を完了させる必要があります。
...
指定されたアドレスを管理する秘密鍵を取得します。  
返された秘密鍵は、[`platform.importKey`](platform-chain-p-chain-api.md#platform-importkey)でユーザーに追加することができます。
...
存在するAVAXの数の上限を返します。これは、トランザクション手数料など使い果たされたトークンを考慮していないため、上限値となります。
...
このトップレベルのフィールド`delegators`はv1.0.1で[非推奨](deprecated-api-calls.md#getcurrentvalidators)となり、v1.0.6で削除されました。その代わりに、今の`validators`の各要素にはそのバリデーターに対するデリゲーターの一覧が含まれています。
...
* `subnetID`は、現在のバリデーターが返されるサブネットです。省略された場合は、一次ネットワークの現在のバリデーターを返します。
* `nodeIDs`は、リクエストする現在のバリデーターのnodeIDのリストです。省略した場合は、現在のすべてのバリデーターを返します。指定したnodeIDが現在のバリデーターのセットに含まれていない場合は、そのノードはレスポンスに含まれません。
* `validators`：
   * `txID`はバリデーターのトランザクションです。
   * `startTime`は、バリデーターがサブネットの検証を開始したUNIX時間です。
   * `endTime`は、バリデーターがサブネットの検証を終了するUNIX時間です。
   * `stakeAmount`は、このバリデーターがステークしたnAVAXの量です。`subnetID`は、一次ネットワークでない場合は省略されます。
   * `nodeID`はバリデーターのノードIDです。
   * `weight`はバリデーターをサンプリングする際のバリデーターの重量です。`subnetID`が一次ネットワークの場合は省略されます。
   ...
   * `delegators`は、このバリデーターに対するデリゲーターのリストです。
      ...
      * `stakeAmount`は、このデリゲーターがステーク下nAVAXの量です。`subnetID`が一次ネットワークではない場合は省略されます。
      * `nodeID`は、検証ノードのノードIDです。
      * `rewardOwner`は、`locktime`、`threshold`、`addresses`の配列を含む`OutputOwners`の出力です。
      * `potentialReward`はステーキングによって得られる潜在的な報酬です
* `delegators`：（**はv1.0.1以降では非推奨です。メソッドのドキュメントの先頭にある注意書きを参照してください。**）
...
* `subnetID`は、現在のバリデーターが返されるサブネットです。省略された場合は、一次ネットワークの現在のバリデーターを返します。
* `nodeIDs`は、リクエストする保留中のバリデーターのnodeIDのリストです。省略した場合は、保留中のすべてのバリデーターを返します。指定したnodeIDが保留中のバリデーターのセットに含まれていない場合は、そのバリデーターはレスポンスに含まれません。
* `validators`：
   * `txID`はバリデーターのトランザクションです。
   * `startTime`は、バリデーターがサブネットの検証を開始したUNIX時間です。
   * `endTime`は、バリデーターがサブネットの検証を終了するUNIX時間です。
...
* `encoding`は、返されるUTXOのフォーマットを指定します。「cb58」または「16進数」のいずれかで、デフォルトは「cb58」です。
...
サブネットのステーキング資産の資産IDを取得します。現在は、一次ネットワークのステーキング資産IDのみを返します。
...
* `ids`は、情報を取得するサブネットのIDです。省略した場合は、すべてのサブネットの情報を取得します。
* `id`は、サブネットのIDです。
...
返されるトランザクションのフォーマットを指定するオプションの`encoding`パラメータです。「cb58」または「16進数」のいずれかを指定できます。デフォルトは「cb58」です。
...
トランザクションのステータスをそのIDで取得します。トランザクションがドロップされた場合、レスポンスには、トランザクションがドロップされた理由の詳細情報を含む`reason`フィールドが含まれます。
...
* ページネーションを使用する場合（つまり`startIndex`が提供されている場合）、UTXOは複数回の呼び出しで一意であることは保証されません。つまり、あるUTXOが最初の呼び出しの結果に現れ、その後、2回目の呼び出しで再び現れる可能性があります。
...
`numFetched`は`limit`と同じなので、取得されなかったUTXOがさらにあるのではないかと考えられます。今度は`startIndex`でこのメソッドを再度呼び出します。
...
ImportTxを構築するために、X ChainからP ChainにエクスポートされたUTXOを取得したいとします。この場合、アトミックUTXOを取得するためには、sourceChain引数で次のGetUTXOを呼び出す必要があります。
...
* `to`は、AVAXがインポートされるアドレスのIDです。これは、対応するX-Chainの`exportAVAX`の呼び出しにおける`to`の引数と同じでなければなりません。
* `sourceChain`は、AVAXがインポートされるチェーンのIDまたはエイリアスです。X-Chainから資金をインポートする場合は、`"X"`を使用します。
* `from`は、この操作に使用するアドレスです。省略した場合は、必要に応じて任意のアドレスを使用します。
* `changeAddr`は、変更が行われた場合に送信されるアドレスです。省略した場合は、ユーザーが管理するいずれかのアドレスに送信されます。
* `username`は、`to`で指定したアドレスを管理するユーザーです。
* `password`は`username`のパスワードです。
...
* `encoding`は、トランザクションバイトのエンコードフォーマットを指定します。「cb58」または「16進数」のいずれかです。デフォルトは「cb58」です。
* `txID`はトランザクションのIDです。
...
* `subnetID`は、サンプリングするサブネットです。省略した場合、デフォルトでは一次ネットワークになります。
