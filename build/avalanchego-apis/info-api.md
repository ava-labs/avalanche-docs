...
ブロックチェーンのエイリアスを指定して、そのIDを取得します。（[`admin.aliasChain`](admin-api.md#admin-aliaschain)を参照。）
...
* `nodeIDs`は、返すべきnodoIDの説明を指定するオプションのパラメータです。このパラメータを空のままにしておくと、すべてのアクティブな接続の説明が返されます。ノードが指定されたnodeIDに接続されていない場合には、そのノードはレスポンスから除外されます。
* `ip`はピアのリモートIPです。
* `publicIP`はピアの公開IPです。
* `nodeID`はピアのプレフィックス付きノードIDです。
* `version`はピアが実行しているバージョンを表示します。
* `lastSent`はピアに送信された最後のメッセージのタイムスタンプです。
* `lastReceived`はピアから受け取った最後のメッセージのタイムスタンプです。
* `benched`はピアがベンチされているチェーンIDを表示します。
* `observedUptime`はピアが観察しているこのノードのアップタイムです。
...
ネットワークが監視している、このノードのアップタイムを返します。
...
* `rewardingStakePercentage`はこのノードがアップタイム要件を上回っているとみなすステークの割合です。
* `weightedAveragePercentage`は、このノードの観察されたすべてのアップタイムのステーク加重平均です。
