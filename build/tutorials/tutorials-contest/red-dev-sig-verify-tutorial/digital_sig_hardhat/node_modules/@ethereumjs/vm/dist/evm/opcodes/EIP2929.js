...
function accessAddressEIP2929(runState, address, chargeGas = true, isSelfdestruct = false) {
    ...
    if (!runState.stateManager.isWarmedAddress(addressStr)) {
        ...
        if (chargeGas) {
            runState.eei.useGas(new ethereumjs_util_1.BN(runState._common.param('gasPrices', 'coldaccountaccess')));
        }
        // Warm: (selfdestruct beneficiary address reads are not charged when warm)
    }
    else if (chargeGas && !isSelfdestruct) {
        runState.eei.useGas(new ethereumjs_util_1.BN(runState._common.param('gasPrices', 'warmstorageread')));
    }
}
...
function accessStorageEIP2929(runState, key, isSstore) {
    ...
    if (slotIsCold) {
        ...
        runState.eei.useGas(new ethereumjs_util_1.BN(runState._common.param('gasPrices', 'coldsload')));
    }
    else if (!isSstore) {
        runState.eei.useGas(new ethereumjs_util_1.BN(runState._common.param('gasPrices', 'warmstorageread')));
    }
