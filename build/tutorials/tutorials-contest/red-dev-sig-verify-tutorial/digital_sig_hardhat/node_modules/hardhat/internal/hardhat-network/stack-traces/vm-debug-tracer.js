...
class VMDebugTracer {
    ...
    async _stepHandler(step, next) {
        errors_1.assertHardhatInvariant(this._messages.length > 0, "Step handler should be called after at least one beforeMessage handler");
    ...
    async _messageToNestedStructLogs(message, address) {
        ...
        for (const [i, messageOrStructLog] of message.structLogs.entries()) {
            if (isStructLog(messageOrStructLog)) {
                ...
                errors_1.assertHardhatInvariant(!Array.isArray(previousStructLog), "There shouldn't be two messages one after another");
                ...
                errors_1.assertHardhatInvariant(!Array.isArray(lastStructLog), "There shouldn't be two messages one after another");
    ...
    async _stepToStructLog(step) {
        ...
        if (step.opcode.name === "SLOAD") {
            ...
            const key = ethereumjs_util_1.setLengthLeft(keyBuffer, 32);
    ...
    async _callDynamicGas(address, value, availableGas, memoryGas, callCost) {
        ...
        if (this._vm._common.gteHardfork("berlin")) {
            ...
            errors_1.assertHardhatInvariant("isWarmedAddress" in stateManager, "The VM should have an EIP2929StateManger when berlin is enabled");
