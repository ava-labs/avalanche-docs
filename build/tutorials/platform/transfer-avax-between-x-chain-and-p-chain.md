...
AVAX, dışa aktarmak için, X-Chain’s yöntemini [`avm.exportAVAX`](../../avalanchego-apis/exchange-chain-x-chain-api.md#avm-exportavax)çağırın.
...
`to``changeAddr`Kullanıcı kontrollerinizin adresine adresi ve herhangi bir değişiklik göndermek için adres adresi olan bir P-Chain adres. Boş `changeAddr`bırakabilirsiniz; eğer boş bırakırsanız, kullanıcınız tarafından kontrol edilen bir adrese dönüşür \(yeni bir P-Chain adresi oluşturulması için talimatlar için [burada](../../avalanchego-apis/platform-chain-p-chain-api.md#platform-createaddress) bakın\).

İhracat ve ithalat işlemleri için bir işlem ücreti ödeyeceğinizi unutmayın. Bu örnekte, işlem ücretinin AVAX olduğunu `.001`varsayalım. Daha sonra yukarıdaki ihracat AVAX; `.006`tüketir; to `.005`gider ve işlem ücreti olarak `.001`yanar.

Gönderdiğiniz miktarın işlem ücretinin çok olduğundan emin olun. Aksi takdirde, AVAX on aktardığınızda, işlem ücretini tüketir ve P-Chain, daha _az _AVAX ile end
...
Bu işlemin kabul edildiğini [`avm.getTxStatus`](../../avalanchego-apis/exchange-chain-x-chain-api.md#avm-gettxstatus)doğrulayabiliriz:
...
Ayrıca AVAX kullanıcımızın yaptığı bir adresten çıkarıldığını kontrol etmek [`avm.getBalance`](../../avalanchego-apis/exchange-chain-x-chain-api.md#avm-getbalance)için arayabiliriz:
...
Düşürülen miktar, `.005`\(AVAX bu örnekteki ihracat miktarı\) artı işlem ücretidir. Eğer kullanıcınız birden fazla X-Chain adreslerini kontrol ediyorsa, AVAX herhangi bir kombinasyonundan gönderilmiş olabilir.

### Adım 2 - AVAX to Aktar

Transferimiz henüz bitmedi. Transferi bitirmek için P-Chain’s [`platform.importAVAX`](../../avalanchego-apis/platform-chain-p-chain-api.md#platform-importavax)yöntemini aramamız gerek.
...
Öyle `Committed`olmalı, yani transfer tamamlandı. Adres dengesini de kontrol edebiliriz:
...
`.001`Gördüğümüz denge, işlem ücreti \(AVAX\) daha az X-Chain `.004`\(AVAX\) tarafından aktarılan miktardır. Şimdi, bu P-Chain adresi tarafından tutulan the Primary Network'ü onaylamak için bir hisse sağlayabiliriz.
...
Bunu yapmak için şöyle [`platform.exportAVAX`](../../avalanchego-apis/platform-chain-p-chain-api.md#platform-exportavax)seslen:
...
`to`X-Chain gönderdiği X-Chain adresi nerede?

Bu işlem kimliğini geri verir, ve işlemin başka bir çağrıyla işlendiğini kontrol [`platform.getTxStatus`](../../avalanchego-apis/platform-chain-p-chain-api.md#platform-gettxstatus)edebiliriz. Gönderdiğiniz miktarın işlem ücretinin çok olduğundan emin olun.

### Adım 2 - AVAX to Aktar

from to To bitirmek için, şöyle [`avm.importAVAX`](../../avalanchego-apis/exchange-chain-x-chain-api.md#avm-importavax)seslen:
...
`to`Bu da bizim çağrımızda belirtilen adresin aynısı.[`platform.exportAVAX`](../../avalanchego-apis/platform-chain-p-chain-api.md#platform-exportavax)

Tıpkı önceki gibi, fonların alındığını doğrulamak [`avm.getBalance`](../../avalanchego-apis/exchange-chain-x-chain-api.md#avm-getbalance)için arayabiliriz. Denge `.003`AVAX işlem ücreti eksi olarak have
