...
Her blok zinciri sanal bir makinenin bir örneğidir. Örneğin, X-Chain AVM ve C-Chain EVM's örneğidir. Avalanche Sanal Makinelerden yeni blok zincirleri oluşturmayı destekler. Bu durumda bir VM eklentisi olan zaman damgası VM'yi kullanacağız. Zaman damgası VM bizim AvalancheGo düğümüyle RPC aracılığıyla iletişim kuracak.

{% page-ref page="create-a-virtual-machine-vm.md" page="create-a-virtual-machine-vm.md" }
...
Her blok zincirinin yaratıldığında bazı genez durumu vardır. Her VM genesis verisinin biçimi ve semantics tanımlar. Zaman damgası CB58 kodlu verileri `genesis`[](create-a-virtual-machine-vm.md#api) verisi olarak kullanır. Kodu ve `kod` çözücü API yöntemleri vardır.
...
Genesis verimiz `fPP1vxkpyLWnH9d6BQA` olacak.
...
`TGas3T58KzdjLHHDMMH2TvrdddhqTji5iZAMZ3RXs2NLpSnhH` ['yi bir Sanal Makine Yaratma VM\ (VM\)](create-a-virtual-machine-vm.md#vm-aliases) olarak kullandığımızı hatırlayın.
...
Bu yeni of yeni örneğiyle etkileşime girebilirsiniz. Buket zincirinin API sonucu, `127.0.0.1:9650/ext/bc/sw813hGSWH8pdU9uzaYy9fYFY2AjD2c9rm64SbApnvjmk'dir`.

Bu zincir kimliğinin `daha` basit API URL'leri için zaman damgası ile veya ne istersen onu da takma adla adlandırabilirsin. Daha fazla bilgi : [admin.aliasChain](https://docs.avax.network/build/avalanchego-apis/admin-api#admin-aliaschain)

### Genesis Blokunu Doğrulayın.

Genesis'te `fP1vxkpyLWnH9d6BQA` genesis verisi olarak belirttik. Şunu doğrulayalım:
...
Gördüğünüz gibi ilk bloğumuzun `zaman` damgası vardır: 0. Ayrıca ebeveyn `kimliği\ (11) \(11111111111111111111111111111111LpoYY\) \(11111111111111111111111111111111LpoYY\)` VM's statik API yöntemiyle genesis verilerini çözelim. Zaman damgası kimliğimizin `zaman damgası` ile sahte olduğunu unutmayın:
...
Genesis verilerinin `cehennem` kuşağı olduğunu görebiliyoruz.

### Yeni Blok Teklif

İçinde bazı verilerle blok zincirine yeni bloklar önerebiliriz.

Önce kodlanmış verileri alalım. Blokların 32 boyunda have sahip olmasını bekliyor. Şifreleme yönteminde `uzunluk` argümanı vardır:
...
Sonuç `veri` alanının içerdiği `veri` alanı qDNkrS9xuyGmaAgdHAjmANSvCKnK5BHvyCybJaFCAq46Z8y içerir. Bu önceki adımda önerilen verilerle aynı veridir.
