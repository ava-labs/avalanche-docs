...
Geçerli olmak için kazık kazığı gereken asgari miktar, 2.000 AVAX \(fiyat zamları olarak zaman içinde azaltılabilir\). Alternatif olarak, doğrulayıcılar ayrıca kullanıcıların maliyetleri çalıştırmak için kendilerine hissesini sunmalarını sağlamak için küçük bir ücret ödeyebilirler. Bir hesap makinesini [kullanarak](https://vscout.io/) bir düğümle çalıştırdığınızda ne kadar kazanacağınızı görebilirsiniz.
...
Eğer Spot Instance VM Pricing oldukça ucuza gelir\) kullanacaksanız ücretsiz bir deneme hesabı can’t onaylandığında bir hata alacaksınız\) bu yüzden Pay-As-You-Go. seçtiğinizden emin **olun.**

![Posta için resim](https://miro.medium.com/max/789/1*TO5Uh07OkH_QdwludEgapg.png)

Bileşen detaylarını girin ve kimlik kimlik işleminin bir parçası olarak kimlik doğrulayın, destek seçeneğini eklemek için ek ödeme yapmak istemiyorsanız destek seçeneğini seçin ve Next'e basın.
...
Ubuntu Sunucusunu seçin \(bu normalde popüler bölüm altında olacak veya pazarda alternatif olarak onu arayacaktır\)
...
İlk olarak, sanal makineye bir isim gir, bu herhangi bir şey olabilir. Ama benim name, ona Avalanche adını verdim. \(aynı zamanda kaynak grubu adını da eşleşmek için otomatik olarak değiştirecek\)
...
Bu makalenin zamanında Kuzey Avrupa'da D2s\_v4 için spot fiyatlandırma saati 0.07975 dolar, veya yılda 698.61 dolar tutar. Fiyat **fiyatıyla saatte 0.01295 dolara düşüyor ve bu da yılda 113.44 dolar ve %83.76 tasarruf ediyor!**
...
Sonraki Seç: Diskleri dize almak için diskleri seçin. Diskler için iki seçenek vardır, ya Premium SSD ya da ayda 64 GB disk maliyetleri ile daha fazla performans sunan ya da düşük performans sunan ve ayda 5 dolar civarında olan standart SSD vardır. Ayrıca Standart SSD ile 10.000 işlem birimi başına 0.002 dolar \(okunur / yazılar\) ödemeniz gerekirken, Premium SSD'ler her şey dahil edilir. Şahsen, daha iyi performans için Premium SSD'yi seçtim ama ayrıca disklerin büyük ihtimalle kullanılması ve uzun vadede daha ucuza da çalışabilmesi nedeniyle.
...
Yerel PC'nizde Avalanche denilen sürücünün kökünde bir dizin oluşturun ve daha önce dizine indirdiğiniz Avalanche\_key.pem dosyasını kaldırın. Sonra dosyanın sağ tıklayın ve Özellikleri seçin. Güvenlik sekmesine git ve alt tabaktan "Gelişmiş" seç.
...
Ssh -i C:\Avalanche\Avalanche\_key.pem pem ubuntu@EnterYourAzureIPHere
...
Düğün, bu anda bir saat yukarıya doğru kaydırmak için zaman alabilir. Bootstrap, düğümlerin indirildiği ve zincirlerin tarihini doğruladığı anlamına gelir. Biraz zaman ver. Düğüm bot kayışı bittiğinde, cevap şöyle olacak:
