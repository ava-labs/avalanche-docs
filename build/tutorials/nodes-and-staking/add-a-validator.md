...
This is the validator’s sampling weight for consensus. If the validator’s weight is 1 and the cumulative weight of all validators in the subnet is 100, then this validator will be included in about 1 in every 100 samples during consensus. Cumulative weight of all validators in the subnet must be at least `snow-sample-size`, so that validators can start validating the subnet. For example if there is only one validator in the subnet, its weight must be at least `snow-sample-size` (default 20). Recall that this is not changeable once issued, so adjust it wisely.
