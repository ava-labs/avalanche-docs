...
Esta página está destinada a ser la única fuente de verdad para cómo seriamos las transacciones atómicas en `Coreth`. Este documento utiliza el formato [de serialización primitivo](serialization-primitives.md) para el embalaje y el [secp256k1](cryptographic-primitives.md#cryptography-in-the-avalanche-virtual-machine) para la identificación de usuario criptográfico.
...
`SECP256K1TransferInput`Las entradas a las transacciones atómicas de Coreth son una de esta cadena o una `TransferableInput`\(que contiene una \) `EVMInput`de otra cadena. `EVMInput`Lo que se utilizará para gastar fondos de esta cadena, mientras que el `TransferableInput`será usado `ExportTx`para importar UTXO atómicas de otra cadena.

### Entrada de la EVM o "EVM Input"

Tipo de entrada que especifica una cuenta de EVM para deducir los fondos como parte de un `ExportTx`.

#### What EVM Input Contains

`amount``assetID``nonce`Una entrada de EVM contiene una `address`, y

* **`Address`**es la dirección de EVM desde la que transferir fondos.
* **`Amount`**es la cantidad del activo que se transferirá \(especificada en nAVAX para AVAX y la denominación más pequeña para todos los demás activos\).
* **`AssetID`**es el ID de un activo para transferir.
* **`Nonce`**es la nonce de la cuenta de EVM exportando fondos.
...
Entrada transferible envuelve una `SECP256K1TransferInput`. Las entradas transferibles o "Transferable inputs" describen un UTXO específico con una entrada de transferencia proporcionada.
...
* **`Input`**es un , tal y `SECP256K1TransferInput`como se define a continuación.
...
Una entrada [de](https://github.com/ava-labs/avalanche-docs/tree/94d2e4aeddbf91f89b830f9b44b4aa60089ac755/build/cryptographic-primitives/README.md#cryptography-in-the-avalanche-virtual-machine) transferencia secp256k1 permite gastar una salida de transferencia de secp256k1 no utilizada.
...
Las salidas a las transacciones de Coreth atómicas son una `EVMOutput`que se añadirá al equilibrio de una dirección en esta cadena o un `TransferableOutput`\(whcih contiene una \) `SECP256K1TransferOutput`para ser movido a otra cadena.

La salida de EVM se utilizará `ImportTx`para agregar fondos a esta cadena, mientras que el `TransferableOutput`será usado para exportar UTXO atómicas a otra cadena.

### Salida de la EVM

Tipo de salida que especifica un cambio de estado que se aplicará a una cuenta de EVM como parte de una `ImportTx`.

#### Que Contiene Una Salida de la EVM

`amount``assetID`Una salida de EVM contiene una `address`, y

* **`Address`**es la dirección de EVM que recibirá los fondos.
* **`Amount`**es la cantidad del activo que se transferirá \(especificada en nAVAX para AVAX y la denominación más pequeña para todos los demás activos\).
* **`AssetID`**es el ID de un activo para transferir.
...
Las salidas transferibles envuelven a con un ID `SECP256K1TransferOutput`de activo.
...
Una salida de transferencia [secp256k1](cryptographic-primitives.md#cryptography-in-the-avalanche-virtual-machine) permite enviar una cantidad de un activo a una colección de direcciones después de un tiempo unix especificado.
...
`destinationChain``inputs`Una ExportTx contiene una `typeID`, `networkID`, `blockchainID`, y `exportedOutputs`.

* **`typeID`**es un int que el tipo para una ExportTx. El typeID para una exportTx es 1.
* **`networkID`**es un int que define a la red de Avalanche esta transacción está destinada a ser emitida. Esto podría referirse a la red de la EVM.
* **`blockchainID`**es una matriz de 32 bytes que define a qué blockchain se emitió esta transacción.
* **`destinationChain`**es una matriz de 32 bytes que define a qué blockchain esta transacción exporta fondos.
* **`inputs`**es una matriz de entradas de EVM para financiar la ExportTx.
* **`exportedOutputs`**es una gama de salidas transferables, que se transferirán a .`destinationChain`
...
`blockchainID``destinationChain``importedInputs`Una importTx contiene una `typeID`, , `networkID`, y `Outs`.

* **`typeID`**es un int que el tipo para una ImportTx. El typeID para un `ImportTx`es 0.
* **`networkID`**es un int que define a la red de Avalanche esta transacción está destinada a ser emitida. Esto podría referirse a la red de la EVM.
* **`blockchainID`**es una matriz de 32 bytes que define a qué blockchain se emitió esta transacción.
* **`sourceChain`**es una matriz de 32 bytes que define qué blockchain desde la que importar fondos.
* **`importedInputs`**es una matriz de entradas TransferableInputs financiar la ImportTx.
* **`Outs`**es una matriz de salidas de EVM que se importan a esta cadena.
...
`SECP256K1Credential`Las credenciales tienen un tipo posible: Cada credencial está emparejada con una entrada. El orden de las credenciales coincide con el orden de las entradas.

### SECP256K1 Credential

Una credencial de [secp256k1](https://github.com/ava-labs/avalanche-docs/tree/94d2e4aeddbf91f89b830f9b44b4aa60089ac755/build/cryptographic-primitives/README.md#cryptography-in-the-avalanche-virtual-machine) contiene una lista de firmas recuperables de 65 bytes
...
Una transacción firmada contiene una credencial sin firma `AtomicTx`y no firmado.

### Que Contiene una Transacción Firmada

`AtomicTx`Una transacción firmada contiene una `CodecID`, y `Credentials`.

* **`CodecID`**El único id de codec válido es`00 00`
* **`AtomicTx`**es una transacción atómica, como se describe arriba.
* **`Credentials`**es una matriz de credenciales. Cada credencial corresponde a la entrada en el mismo índice en el AtomicTx
