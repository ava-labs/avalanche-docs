...
このアップグレードによる変更は、2021年9月22日午後**5時EDT（米国東部標準時）／午後9時UTC（協定世界時）**にMainnet上で有効となります。この変更が有効となる前に、ご自分のノードをアップグレードしてください。さもなければノード上のアップタイムを失うことになる可能性があります。
...
**このリリースではMainnetを実行することはできません。Mainnet構成で実行しようとすると「mainnet is not supported（Mainnetはサポートされていません）」と表示されることにご注意ください。**

このアップグレードでは、コンテンションリミッターをC-ChainとP-Chainに追加し、C-Chainにブロックベースの手数料を導入し、C-Chainでのダイナミック手数料パラメータの調整を行います。

このアップグレードによる変更は、2021年9月16日午後5時EDT（東部標準時）にFuji Testnet上で有効になります。Fujiが更新され、検証された後、Mainnnetと互換性を持つリリースが公開されます。

**すべてのFujiノードは、2021年9月16日午後5時EDT（東部標準時）までにアップグレードする必要があります。**
...
**このリリースでは、Mainnetを実行することはできません。Mainnnet構成で実行しようとすると「このノードバージョンではMainnnetはサポートされていません」と表示されますのでご注意ください。Mainnetのノードを実行する場合、来週正式リリース公開まで何もする必要は必要ありません。**
...
**インストール済みのノードのバージョンがv1.4.4以下の場合は、このノードはデータベースの移行を行います。データベースの移行についての詳細は、v1.4.5のリリースノートを参照してください**。インストール済みのノードのバージョンがv1.4.5である場合、このノードは既存のデータベースを使用するため、データベースの移行を行う必要はありません。

このパッチです。

* 高い持続性を持つDB書き込みの原因となっていた、 P-Chainメモリプールへの無効なトランザクション発行を削除します。
* データベースディレクトリ内のデータベース以外のファイルやフォルダを無視するようになりました。これにより、特に、macOSで報告されていた.DS_Storeファイルのエラーが修正されます。
...
**この更新は、通常のバージョンのアップデートより多くのものが含まれます。より詳細な手順とFAQは**[**こちら**](https://docs.avax.network/build/release-notes/avalanchego-v1.4.5-database-migration)**をご覧ください。**
...
{% hint style="danger" %}
**このパッチでは、以前のリリースとの後方互換性はありませんのでご注意ください。今回のアップグレードでは、X、C、P chain間のインターチェンジ転送に関するパフォーマンスの問題が修正されています。コミュニティの皆様には、ご自身のノードが影響を受けないように、できるだけ早くアップグレードされることをお勧めします。また、アップグレード後にノードの接続に数分かかることがありますが、このプロセスは中断せずに完了させてください。**
{% endhint %}
...
{% hint style="danger" %}**このアップグレードは、以前のリリースとの後方互換性はありませんのでご注意ください。アップグレードは、12月7日（月）午後11時UTC（東部標準時）（午後6時EST）までに実施してください。当初12月中旬頃に予定していたこのアップグレードは、トークンのロック解除に関する重要なバグを修正するために迅速に処理されます。コミュニティの皆様には、ご自身のノードが影響を受けないように、できるだけ早くアップグレードされることをお勧めします。**
{% endhint %}
