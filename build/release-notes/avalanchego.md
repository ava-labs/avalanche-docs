...
## v1.6.5（[GitHubでの表示](https://github.com/ava-labs/avalanchego/releases/tag/v1.6.5)）

このバージョンは、[v1.6.0](https://github.com/ava-labs/avalanchego/releases/tag/v1.6.0)との後方互換性があります。これは任意ですが、推奨されています。

**ブートストラッピング**

- チェーンがブートストラップの実行フェーズにある場合は、そのチェーンにインバウンドメッセージをドロップします。
- それらに接続できない場合はビーコンノードIDを表示します。

**メトリック**

- `avalanche_{ChainID}_bootstrap_finished`を追加し、チェーンがブートストラップを完了した場合は1、それ以外は0になります。

**APIs**

- ローカルノードのネットワークビューの報告を試みる`info.uptime`API呼び出しを追加しました。
- `info.peers`の各ピアの結果に`observedUptime`を追加しました。

**ネットワーク**

- Pongメッセージに報告されたアップタイムを追加し、ネットワークで表示されるローカルノードのアップタイムをより追跡できるようにしました。
- 競合状態になることを回避するために、リクエストタイムアウトレジストリをリファクタリングしました。


## v1.6.4（[GitHubでの表示](https://github.com/ava-labs/avalanchego/releases/tag/v1.6.4)）

このバージョンは、[v1.6.0](https://github.com/ava-labs/avalanchego/releases/tag/v1.6.0)との後方互換性があります。これは任意ですが、推奨されています。

**構成**

- ピアの最大平均インバウンド帯域幅使用量を指定するフラグ`throttler-inbound-bandwidth-refill-rate`を追加しました。
- ピアの最大インバウンド帯域幅使用量を指定するフラグ`throttler-inbound-bandwidth-max-burst-size`を追加しました。

**ネットワーク**

- 他のゴシップ呼び出しと同じメカニズムを使用できるようにピアリストゴシップを更新しました。
- 最近の帯域幅使用量に基づくインバウンドメッセージスロットルを追加しました。

**メトリック**

- `avalanche_{ChainID}_handler_gossip_{count,sum}`を`avalanche_{ChainID}_handler_gossip_request_{count,sum}`に更新しました。
- `avalanche_{ChainID}_lat_get_accepted_{count,sum}`を`avalanche_{ChainID}_lat_accepted_{count,sum}`に更新しました。
- `avalanche_{ChainID}_lat_get_accepted_frontier_{count,sum}`を`avalanche_{ChainID}_lat_accepted_frontier_{count,sum}`に更新しました。
- `avalanche_{ChainID}_lat_get_ancestors_{count,sum}`を`avalanche_{ChainID}_lat_multi_put_{count,sum}`に更新しました。
- `avalanche_{ChainID}_lat_pull_query_{count,sum}`と`avalanche_{ChainID}_lat_push_query_{count,sum}`を`avalanche_{ChainID}_lat_chits_{count,sum}`に結合しました。
- `avalanche_{ChainID}_app_response_{count,sum}`を追加しました。
- 追加した`avalanche_network_bandwidth_throttler_inbound_acquire_latency_{count,sum}`
- 追加した`avalanche_network_bandwidth_throttler_inbound_awaiting_acquire`
- 追加した`avalanche_P_vm_votes_won`
- 追加した`avalanche_P_vm_votes_lost`

**インデクサ**

- メソッド`GetContainerByID`をクライアント実装に追加しました。
- クライアントメソッドは、コンテナの`string`表現ではなく、`[]byte`を返すようになりました。

**C-Chain**

- Gethの依存関係を1.10.11に更新しました。
- 新しい管理APIを追加し、ログレベルを更新し、パフォーマンスの測定を行えるようにしました。
- 新しい`--allow-unprotected-txs`フラグを追加し、EIP-155反射保護なしにトランザクション発行を許可できるようにしました。

**サブネットとカスタムVM**

- すべての可能なチェーンが`--staking-enabled=false`ネットワーク上で実行できるようにしました。


## v1.6.3（[GitHubでの表示](https://github.com/ava-labs/avalanchego/releases/tag/v1.6.3)）

このバージョンは、[v1.6.0](https://github.com/ava-labs/avalanchego/releases/tag/v1.6.0)との後方互換性があります。これは任意ですが、推奨されています。

**構成オプション**

- `--inbound-connection-throttling-max-conns-per-sec`のデフォルト値を`256`に更新しました。
- `--meter-vms-enabled`のデフォルト値を`true`に更新しました。
- `--staking-disabled-weight`のデフォルト値を`100`に更新しました。

**メトリック**

- メッセージが実際にブロックしている場合、`avalanche_network_buffer_throttler_inbound_awaiting_acquire`の動作をインクリメントのみに変更しました。
- メッセージが実際にブロックしている場合、`avalanche_network_byte_throttler_inbound_awaiting_acquire`の動作をインクリメントのみに変更しました。
- `Block/Tx`メトリックスを`meterVM`に追加しました。
   - `avalanche_{ChainID}_vm_metervm_build_block_err_{count,sum}`を追加しました。
   - `avalanche_{ChainID}_vm_metervm_parse_block_err_{count,sum}`を追加しました。
   - `avalanche_{ChainID}_vm_metervm_get_block_err_{count,sum}`を追加しました。
   - `avalanche_{ChainID}_vm_metervm_verify_{count,sum}`を追加しました。
   - `avalanche_{ChainID}_vm_metervm_verify_err_{count,sum}`を追加しました。
   - `avalanche_{ChainID}_vm_metervm_accept_{count,sum}`を追加しました。
   - `avalanche_{ChainID}_vm_metervm_reject_{count,sum}`を追加しました。
   - `avalanche_{DAGID}_vm_metervm_parse_tx_err_{count,sum}`を追加しました。
   - `avalanche_{DAGID}_vm_metervm_get_tx_err_{count,sum}`を追加しました。
   - `avalanche_{DAGID}_vm_metervm_verify_tx_{count,sum}`を追加しました。
   - `avalanche_{DAGID}_vm_metervm_verify_tx_err_{count,sum}`を追加しました。
   - `avalanche_{DAGID}_vm_metervm_accept_{count,sum}`を追加しました。
   - `avalanche_{DAGID}_vm_metervm_reject_{count,sum}`を追加しました。

**Coreth**

- callTracerフォルトハンドリング修正を適用しました。
- ランタイム環境でマルチコインファンクションを初期化しました。

**ProposerVM**

- `Delay`ブロックを`--staking-enabled=false`ネットワーク上で更新し、`0`にしました。


## v1.6.2（[GitHubでの表示](https://github.com/ava-labs/avalanchego/releases/tag/v1.6.2)）

このバージョンは、[v1.6.0](https://github.com/ava-labs/avalanchego/releases/tag/v1.6.0)との後方互換性があります。これは任意ですが、推奨されています。

**構成オプション**
* `--coreth-config`を削除しました。[こちら](../references/command-line-interface.md#c-chain-config)を参照してください。
* `--throttler-inbound-node-max-processing-msgs`を追加しました。[こちら](../references/command-line-interface.md#message-rate-limiting-throttling)を参照してください。
* `--db-config-file`を追加しました。[こちら](../references/command-line-interface.md#database-config)を参照してください。

**API**
* APIメソッド`avm.exportAVAX`は削除されました。代わりに`avm.export`を使用します。
* APIメソッド`avm.importAVAX`は削除されました。代わりに`avm.import`を使用します。
* APIメソッド`info.peers`は、現在オプションとして`PublicIP`フィールドを扱い、有効なIPが提供されたときのみ追加されます。
* APIクライアント`platform.getValidatorsAt`は追加されました。
* APIクライアント`admin.lockProfile`は、正しく`lockProfile`を呼び出すように修正されました。
* APIクライアント`health.health`は、不健全なサーバーからのレスポンスを適切に処理するために修正されました。
* Health Check APIからのレスポンスが改善され、より記述的になりました。

**ベンチリスト**
* バリデータが応答しない最小時間とバリデータがベンチする最大時間を変更しました。以前のバージョンではそれぞれ5分と30分でしたが、このバージョンでは2.5分と15分としました。

**データベース**
* ユーザーが`--db-config-file`フラグでデータベースの構成を指定できるようになりました。

**サブネット**
* クライアントがサブネットをプライベートに設定してメンバーシップを承認されたバリデータののみに限定できる機能を追加しました。

**ネットワーク**
* インバウンド全体のメッセージへの割り当てのデフォルトサイズを32 MiBから6 MiBに変更しました。
* アウトバウンド全体のメッセージへの割り当てのデフォルトサイズを32 MiBから6 MiBに変更しました。
* インバウンド全体のメッセージへの割り当てからノードが取得できるデフォルトの最大バイト数を4 MiBから2 MiBに変更しました。
* アウトバウンド全体のメッセージへの割り当てからノードが取得できるデフォルトの最大バイト数を4 MiBから2 MiBに変更しました。
* 追加のインバウンドメッセージレート制限を追加しました。そのピアからの処理量が`--throttler-inbound-node-max-processing-msgs`を下回るまで、ノードはピアからそれ以上のメッセージを読めなくなります。
* AppGossipメッセージがゴシップされる非バリデータ数のデフォルトを2から0に変更しました。
* AppGoshipのメッセージがゴシップされるバリデータ数のデフォルトを４から6に変更しました。
* 一様にランダムに行うだけでなく、VMが特定のバリデータをゴシップできる機能を導入しました。
* いったん切断されたノードに再接続できないノードをつくりだしてしまうという問題を修正しました。

**ProposerVM**
* 悲観的なP-Chainの高さの遅れを導入し、P-Chainブロックの大量発行時の安定性を改善しました。
* 要求されたブロック遅延を正しく適用しました。

**メトリック**
* X-ChainとP-ChainからAPIリクエストヒストグラムメトリックを削除しました。
* P-Chainメモリプールメトリックを追加しました。
* platformvmに`validator_sets`メトリックを追加しました。

**その他**
* ノードが起動後すぐにストップした際の不都合なシャットダウンを避けるため、ノードの起動とシャットダウンをリファクタリングしました。
* 割り当てられたバイト数を正確に追跡するために、P-Chainのメモリプールを修正しました。
* C-Chainをアップグレードし、geth1.10.9を実行しました。
* C-Chainのためのabigenをサポートしました。
* C-Chain上でのプリイメージサポートのためのサポートを追加しました。
* C-Chain上での手数料履歴エンドポイントのサポートを追加しました。
* GRPCテストをよりよくサポートするためにIDエイリアスをリファクタリングしました。
* エンドツーエンドテストブランチマッチアップロジックを削除しました。
* データベースミグレーションプロセスマネージャの非推奨メインエントリーポイントを削除しました。

## v1.6.1（[GitHubでの表示](https://github.com/ava-labs/avalanchego/releases/tag/v1.6.1)）

このバージョンは、[v1.6.0](https://github.com/ava-labs/avalanchego/releases/tag/v1.6.0)との後方互換性があります。これは任意ですが、推奨されています。

**アップグレード**

* サブネット構成を指定できる機能を追加しました。
* 多様な新しいネットワーク構成値を追加しました。
* ネットワークライブラリからレガシーメッセージを削除しました。
* ローカルネットワーク上でAddValidatorトランザクションに影響を与えるP-Chainメモリプールのバグを修正しました。
* バリデータの修正された数とすべてのピアにゴシップするトランザクションゴシップルールを変更しました。
* Health APIから非推奨`getLiveness`メソッドを削除しました。
* 非バリデータ間の接続を無効にするための設定オプションを追加しました。

**注意**

以下のものは推奨されておらず、使用できなくなります。これらは将来のバージョンで削除される可能性があります。

* APIメソッド`avm.exportAVAX`は削除され、`avm.export`に代わります。
* APIメソッド`avm.importAVAX`は削除され、`avm.import`に代わります。
* 構成オプション`coreth-config`は削除され、[チェーン構成ファイル](../references/command-line-interface.md#c-chain-config)に代わります。

## v1.6.0（[GitHubでの表示](https://github.com/ava-labs/avalanchego/releases/tag/v1.6.0)）

**この変更は、以前のリリースとの後方互換性はありません。**

このアップグレードでは、コンテンションリミッターをC-ChainとP-Chainに追加し、C-Chainにブロックベースの手数料を導入し、C-Chainでのダイナミック手数料パラメータの調整を行います。

このアップグレードによる変更は、**2021年9月22日午後5時（東部標準時（夏時間））／午後9時（協定世界時）にMainnet上で**有効になります。この変更が有効となる前に、ご自分のノードをアップグレードしてください。さもなければノード上のアップタイムを失うことになる可能性があります。

さらなる詳細は[こちら](https://medium.com/avalancheavax/apricot-phase-four-snowman-and-reduced-c-chain-transaction-fees-1e1f67b42ecf)をご覧ください。

**Go**

AvalancheGoの構築に必要なGoのバージョンは、Go 1.16.8以上になります。

**バグ修正**

タイムアウトマネージャー起動時のレースコンディションを修正しました。

**アップグレード**

* P-ChainとC-Chain上に[Snowman++](https://github.com/ava-labs/avalanchego/blob/v1.6.0-fuji/vms/proposervm/README.md)を導入しました。
* VM<->VM 通信層を使用して、[メモリプールゴシップをP-Chain](https://github.com/ava-labs/avalanchego/blob/v1.6.0-fuji/vms/platformvm/README.md)とC-Chainに導入しました。
* C-Chainブロックにブロックベース手数料を追加しました。
* C-Chainダイナミック手数料メカニズムで最低ガス価格を25nAVAXに、最高ガス価格を1000nAVAXに設定しました。
* レート制限をしたインカムコネクション

**新しいメトリック**

* `avalanche_C_blks_built`/`avalanche_P_blks_built`：C-ChainとP-Chain上それぞれでローカルに構築されたブロック数。
* `avalanche_C_blks_builds_failed`/`avalanche_P_blks_builds_failed`：C-ChainとP-Chain上それぞれでフェイルしたBuildBlockへの呼び出し数。

**構成オプション**

* `inbound-connection-throttling-max-conns-per-sec`フラグを追加しました。（[構成ドキュメント](../references/command-line-interface.md)を参照してください。）
* `inbound-connection-throttling-max-recent`フラグを非推奨としました。このフラグは今は無視されます。

## PRE_RELEASE v1.6.0-fuji（[GitHubでの表示](https://github.com/ava-labs/avalanchego/releases/tag/v1.6.0-fuji)）

**このリリースではMainnetを実行することはできません。Mainnet構成で実行しようとすると「mainnet is not supported（Mainnetはサポートされていません）」と表示されるのでご注意ください。**

このアップグレードでは、コンテンションリミッターをC-ChainとP-Chainに追加し、C-Chainにブロックベースの手数料を導入し、C-Chainでのダイナミック手数料パラメータの調整を行います。

このアップグレードによる変更は、2021年9月16日午後5時EDT（東部標準時）にFuji Testnet上で有効になります。Fujiが更新され、検証された後、Mainnnetと互換性を持つリリースが公開されます。

**すべてのFujiノードは、2021年9月16日午後5時（東部標準時（夏時間））までにアップグレードする必要があります。**
...
* C-Chainブロックにブロックベース手数料を追加しました。
* C-Chainダイナミック手数料メカニズムで最低ガス価格を25nAVAXに、最高ガス価格を1000nAVAXに設定しました。
* 構築されたブロック数とブロック構築に失敗した数のメトリックを追加しました。

## v1.5.3（[GitHubでの表示](https://github.com/ava-labs/avalanchego/releases/tag/v1.5.3)）

このバージョンは、[v1.5.0](https://github.com/ava-labs/avalanchego/releases/tag/v1.5.0)と後方互換性があります。

**アップタイム**

* ステーキング報酬を受け取るための最低アップタイム要件を60%から80%に変更しました。

**ネットワーク**

* 次の3つの新しいネットワークメッセージを追加しました：`AppRequest`、`AppResponse`と`AppGossip`です。これらのメッセージにより、ブロックチェーンのインスタンスは、VMで定義された任意のデータを相互に送信できるようにします。以前は、ブロックチェーンのインスタンスは、コンセンサスメッセージ（`Put`や`PushQuery`など）を送信することでしか相互に通信できませんでした。`snow/engine/common/engine.go`を参照してください。
* `Pong`メッセージを受け取ると、そのバージョンと互換性がない場合、その送信者から切断されます。
* 明確にするため、`common.Sender`に`Send`で名づけられたメソッドを前につけます。（例：　`Put`->`SendPut`）

**P-Chain**

* ブロックごとにバリデータ重量を追跡する機能を追加しました。
* `GetValidatorsAt`APIメソッドを追加し、指定されたP-Chainの高さに設定されたサブネット（あるいはプライマリネットワーク）のバリデータを検索できるようにしました。

**C-Chain**

* Geth v1.10.8からの変更を組み込みます。
* Ancientsへの参照を削除します。

**コンセンサス**

* `Timestamp()`メソッドを`snowman.Block`インターフェースに追加しました。

**ローカルネットワーク**

* ローカルジェネシスにおけるバリデータの起動時間をアップデートしました。v1.5.3以前のバージョンでのローカル構成で指定されていたバリデータの終了時間は、2021年9月10日0時0分0秒（UTC）です。**このため、この時間以降でもローカルネットワークを実行するために、AvalancheGo v1.5.3にアップグレードする必要があります。**

**構成オプション**

* AvalancheGo構成オプション`consensus-app-gossip-size`を追加しました。これにより、`AppGossip`メッセージがゴシップされるピアの数を定義できるようになります。
* C-Chain構成オプションを追加しました`log-level`。オプションは：`"trace"`、`"debug"`、`"info"`、`"warn"`、`"error"`、`"crit"`です。デフォルトは`"debug"`です。（以前と同様です。）

## v1.5.2（[GitHubでの表示](https://github.com/ava-labs/avalanchego/releases/tag/v1.5.2)）

この更新は、[v1.5.0](https://github.com/ava-labs/avalanchego/releases/tag/v1.5.0)との後方互換性があります。v1.5.0リリースの更新予定時間をご確認ください。
...
この更新は、[v1.5.0](https://github.com/ava-labs/avalanchego/releases/tag/v1.5.0)との後方互換性があります。v1.5.0リリースのネットワーク更新予定時間をご確認ください。
...
* ハンドシェイクメッセージに`subnetID`を追加しました。これにより、ノードが同期しようとしているサブネットについてピアに通知されます。
...
このアップグレードによる変更は、2021年8月24日午前10時EDT（東部標準時）にMainnet上で有効になります。この変更が有効となる前に、ご自分のノードをアップグレードしてください。さもなければノード上のアップタイムを失うことになる可能性があります。
...
**このリリースでは、Mainnetを実行することはできません。Mainnnet構成で実行しようとすると、「このノードバージョンではMainnnetはサポートされていません」と表示されますのでご注意ください。Mainnetのノードを実行する場合は、来週の正式リリース公開まで何もする必要ありません。**

**この変更は、以前のリリースとの後方互換性はありません。**

今回のバージョンアップでは、C-Chainにダイナミック手数料が追加されたほか、様々な改良が加えられています。

このアップグレードによる変更は、2021年8月16日午後3時EDT（東部標準時）にFuji Testnet上で有効になります。Fujiが更新され、検証された後、Mainnnetに対応するリリースが公開されます。
...
この更新は後方互換性があります。これは任意ですが、推奨されています。
...
多くのPrometheusメトリックの名称が変更され、多くのヒストグラムが2つのゲージに置き換えられました。更新されたGrafanaダッシュボードは[こちら](https://github.com/ava-labs/avalanche-docs/tree/master/dashboards)をご覧ください。

今回のリリースでは、`utils/metric`パッケージにヘルパーメソッドも追加されています。

**RocksDB**

このビルドスクリプトの実行時に、RocksDBはデフォルトでは構築されなくなりました。したがって、一般公開されているバイナリには含まれません。RocksDBでAvalancheGoを構築するには、ターミナルで`export ROCKSDBALLOWED=1`を実行し、次に`scripts/build.sh`を実行します。`--db-type=rocksdb`を使用する前に必ずこれを行う必要があります。

RocksDBデータベースは、サブディレクトリ`rocksdb`にファイルを保存／検索するようになりました。これまでRocksDBを使っていた場合は、既存のファイルを移動する必要があることに注意してください。

**メッセージの圧縮**

ノードは、一部のP2Pメッセージを圧縮するようになりました。ピアのバージョンがv1.4.11以上の場合、ピアに送信されるPut、Push Query、Peer List、Multiputメッセージは、ネットワークに送信される前にgzipで圧縮されます。これによりAvalancheGoの帯域幅使用量を削減します。
...
{% hint style="warning" %}
今回の更新は後方互換性があります。これは任意ですが、推奨されています。{% endhint %}
...
* プルーニングを有効にしてC-Chainを実行できるようにしました。現在、デフォルトではプルーニングは無効になっています。
...
* AVMの手数料定義を変更し、チェーンのネイティブ資産で支払われる手数料を強制するようにしました。これによりX-Chainの動作が変更されることはありませんが、他のAVMインスタンスが使用可能になります。
* 特定のチェーンに設定を指定する機能を追加しました。これにより`coreth-config`CLIパラメータは非推奨となります。
...
{% hint style="warning" %}
今回の更新は後方互換性があります。これは任意ですが、推奨されています。このパッチには、パフォーマンス改善とバグ修正が含まれています。{% endhint %}

インストール済みのノードのバージョンがv1.4.4以下の場合、このノードはブロックの処理を停止している場合があります。この更新により、ノードの修復とデータベースの移行が行われます。データベースの移行についての詳細は、[v1.4.5のデータベース移行に関するノート](avalanchego-v1.4.5-database-migration.md)を参照してください。インストール済みのノードのバージョンがv1.4.5以上の場合、このノードは既存のデータベースを使用するため、データベースの移行を行う必要はありません。
...
**インストール済みのノードのバージョンがv1.4.4以前の場合、このノードはデータベース移行を行います。データベース移行についての詳細は、v1.4.5のリリースノートを参照してください。**インストール済みのノードのバージョンがv1.4.5である場合、このノードは既存のデータベースを使用するため、データベースの移行を行う必要はありません。

このパッチです。

* 高い持続性を持つDB書き込みの原因となっていた、 P-Chainメモリプールへの無効なトランザクション発行を削除します。
* データベースディレクトリ内のデータベース以外のファイルやフォルダを無視するようになりました。これにより、特にmacOSで報告されていた.DS_Storeファイルのエラーが修正されます。
...
**この更新は、通常のバージョンのアップデートより多くのものが含まれます。より詳細な手順とFAQはこちらをご覧ください**[****](https://docs.avax.network/build/release-notes/avalanchego-v1.4.5-database-migration)**。**

{% hint style="warning" %}
今回の更新は後方互換性があります。これは任意ですが、推奨されています。このパッチには、大幅なパフォーマンスの改善とその他多くの更新が含まれています。{% endhint %}
...
次の引数`"default"`は、以前キーワードとして扱われていました。現在では、`"default"`はフラグの意図した値として扱われる試みがされています。デフォルトの動作を維持する場合には、フラグを指定しないでください。
...
次の引数`""`は、以前キーワードとして扱われていました。現在では、`""`はフラグの意図した値として扱われる試みがされています。デフォルトの動作を維持する場合には、フラグを指定しないでください。
...
{% hint style="danger" %}**この変更は、以前のリリースとの後方互換性はありませんのでご注意ください。**
...
{% hint style="warning" %}このアップグレードでは、Ethereum BerlinのアップグレードをC-Chainに適用し、新しいAVMエンドポイントを追加するとともに、様々な安定性の向上を図っています。コミュニティの皆様には、ご自身のノードを良好な状態に保つために、できるだけ早くアップデートされることをお勧めします。
...
{% hint style="danger" %}
今回の更新は後方互換性があります。これは任意ですが、推奨されています。このパッチには、セキュリティの改善、バグの修正、およびモニタリングの改善が含まれています。{% endhint %}
...
* デリゲーションキャップの計算を修正し、デリゲーターを削除する前に現在のデリゲーションの最大値を更新するようにしました。これにより、デリゲーションキャップが常に適用されるようになりました。
...
{% hint style="danger" %}
今回の更新は後方互換性があります。これは任意ですが、推奨されています。このパッチには、安定性、モニタリングの改善、およびマイナーなバグの修正が含まれます。{% endhint %}
...
このアップグレードでは、C-Chainガス価格の削減、C-Chainガスの払い戻しの削除、そして様々なセキュリティの改善が行われています。

コミュニティの皆様には、ご自身のノードを良好な状態に保つために、できるだけ早くアップデートされることをお勧めします。{% endhint %}
...
{% hint style="danger" %}
今回の更新は後方互換性があります。これは任意ですが、推奨されています。このパッチには、安定性とモニタリングの改善が含まれています。{% endhint %}
...
{% hint style="danger" %}
今回の更新は後方互換性があります。これは任意ですが、推奨されています。このパッチには、安定性、パフォーマンス、およびモニタリングに関する改善が含まれています。{% endhint %}
...
{% hint style="danger" %}
今回の更新は後方互換性があります。これは任意ですが、推奨されています。このパッチには、安定性、パフォーマンス、およびモニタリングに関する改善が含まれています。
...
* 追加されたコマンドライン引数は、主に健全性のチェックの設定で構成されています。また、ネットワーク待ち時間の計算方法を変更したことで、いくつかのコマンドライン引数の名前が変わりました。
...
{% hint style="danger" %}**このパッチでは、以前のリリースとの後方互換性はありませんのでご注意ください。今回のアップグレードでは、X、C、P chain間のインターチェンジ転送に関するパフォーマンスの問題が修正されています。コミュニティの皆様には、ご自身のノードが影響を受けないように、できるだけ早くアップグレードされることをお勧めします。また、アップグレード後のノードの接続には数分余計にかかることがありますが、このプロセスは中断せずに完了させてください。**{% endhint %}
...
{% hint style="danger" %}
今回の更新は後方互換性があります。これは任意ですが、推奨されています。このパッチには、安定性の向上が含まれています。{% endhint %}
...
{% hint style="danger" %}
今回の更新は後方互換性があります。これは任意ですが、推奨されています。このパッチには、CLIのアップグレード、APIのバグ修正、安定性の向上、パフォーマンスの改善が含まれています。{% endhint %}
...
{% hint style="danger" %}この更新は任意ですが、推奨されています。このパッチには、APIに関連するマイナーなバグ修正が含まれています。{% endhint %}
...
{% hint style="danger" %}この更新は任意ですが、推奨されています。このパッチには、バグ修正とパフォーマンスの向上が含まれています。{% endhint %}
...
{% hint style="danger" %}**このアップグレードは、以前のリリースとの後方互換性はありませんのでご注意ください**。アップグレードは、12月7日（月）午後11時（協定世界時）（午後6時（東部標準時））までに必ず実施してください。当初12月中旬頃に予定していたこのアップグレードは、トークンのロック解除に関する重要なバグを修正するために前倒しで処理されます。コミュニティの皆様には、ご自身のノードが影響を受けないように、できるだけ早くアップグレードされることをお勧めします。{% endhint %}
...
{% hint style="danger" %}このリリースには、[こちら](https://docs.avax.network/build/apis/deprecated-api-calls)に記載されている変更（ブレーキングチェンジ）が含まれていることに注意してください。このリリースでは、platform.getTxStatusおよびplatform.getCurrentValidatorsのデフォルトのレスポンスフォーマットが変更されています。この更新は任意ですが、推奨されています。このパッチには、パフォーマンスの改善と一部生活の質にかかわる改善が含まれています。{% endhint %}
...
{% hint style="danger" %}
この後にリリースされるv1.0.6には、[こちら](https://docs.avax.network/build/apis/deprecated-api-calls)で説明されている変更（ブレーキング・チェンジ）が含まれることにご注意ください。具体的には、`platform.getTxStatus`と`platform.getCurrentValidators`のレスポンスフォーマットが変わります。{% endhint %}

このリリースv1.0.5での変更点は、以前のリリースとの後方互換性があります。この更新は任意ですが、推奨されています。このパッチには、パフォーマンスの向上と一部の生活の質にかかわる改善が含まれています。
...
{% hint style="danger" %}この更新は任意ですが、推奨されています。このパッチには、一部の生活の質にかかわる改善とパフォーマンスの向上が含まれています。今回の更新では、CLIのパラメータを - または - で指定するのではなく、 - で指定する必要があることに注意してください。例えば、`-public-ip=127.0.0.1`は許可されなくなり、`--public-ip=127.0.0.1`として指定する必要があります。それ以外は、この更新は、後方互換性があります。{% endhint %}
