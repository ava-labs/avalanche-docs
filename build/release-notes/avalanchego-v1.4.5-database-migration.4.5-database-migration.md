...
Bu makale, your AvalancheGo < v1.4.5 ile AvalancheGo >= v1.4.5 arasında değiştirirken uygulanabilir. Bu makale v1.4.5 için yazılmış olsa da aşağıdaki sürümle ilgili referanslar olsa da, AvalancheGo v1.4.4.4'ten AvalancheGo v1.6, v1.4.7, v1.7. vb.. Bu belgeyi okuduğunda, güncellediğiniz sürüm ile v1.4.5 değiştirilmeyecek olan veritabanı alt dizinine referans olarak eklenince değiştirin. \)
...
* Lütfen bu belgeyi okuyun. Göç sırasında your başarılı bir şekilde göç ettiğinden ve sağlıklı kaldığından emin olun. Soruna cevap vermezse, [Discord](https://chat.avalabs.org/) servisimize git, işaretlenmiş mesajları okuyup sorunuzu arayın. Hala yardıma ihtiyacınız varsa, lütfen sorunlu kanalda bulunun.
...
Bilgisayarınızda var olan bir veritabanı varsa AvalancheGo v1.4.5 çalıştırdığınızda aslında 2 düğümle başlar. Bir tanesi v1.4.4 çalıştırılacaktır ve bu da "eski" veritabanı sürümü \(v1.0.0\) kullanacaktır. Diğer araç ise "yeni" veritabanı biçimi \(v1.4.5) kullanılarak çalıştırılacak.
...
`Bayrak eklentisi` vermemelisin. AvalancheGo, kurmak için kurucu betiği kullanmışsanız, bu bayrağı AvalancheGo servis dosyanızdan silmeniz gerekir. Şu [nota](avalanchego-v1.4.5-database-migration.md#note-for-the-nodes-installed-with-installer-script) bak.

## Kaynak Kullanımı

Göç sırasında, iki düğümler de çalıştığında AvalancheGo her zamankinden daha fazla sistem kaynaklarını tüketecek.

Göç işlemi tamamlandığında bilgisayarınızda iki tane baglı veri tabanı olacak. Bilgisayarınızdaki serbest disk boşluğunun tam bağlı bir veritabanı \(~38 GB\) büyüklüğünden daha fazla olduğundan emin olun. Bilgisayarınızda en az 200 GB disk alanı to adamanızı öneriyoruz. AvalancheGo şu anda bu miktarın sadece bir kısmını kullanırken biz disk kullanımını öngörüyoruz, bir çözeltme uygulamadan önce önce disk kullanımını artıracağız.
...
* [Yazıtlar](https://docs.avax.network/build/tutorials/nodes-and-staking/set-up-node-with-installer#node-upgrade) ile birlikte çalıştırın. / _`./avalanchego-installer.sh`_
...
* AvalancheGo bir hizmet olarak çalıştırırsanız, ki bu büyük tavsiyemiz, _`--eklenti dir`_ bayrağının _`AvalancheGo`_ dosyasında olmadığını doğrulayın. Eğer mevcut değilse, aşağıdaki paragrafı atlayabilirsiniz. Eğer varsa, aşağıdaki talimatları uygulayın.

   Konsolda komutu girin: _`sudo nano /etc/systemd/system/avalanchego.service`_   Editörde, _`with`_ başlayacak satırı bulun ve şu kısmı silin: _`part:`_ ctrl-x ve y'ye basarak değişiklikleri kaydeder.

   Değişiklikleri uygulamak için, komutu çalıştır:   _`sudo systemctl daemon-reload` yükleme_   Sonunda düğümü şöyle yeniden başlat:   _`sudo systemctl avalanchego yeniden başlat`_
...
* Yeni veritabanının nüfusa sahip olduğu düğümler için kayıtlar _`$HOME/.avalanchego/logs/fetch-only`_ altında bulunabilir.
* Bu mesajlar veritabanı göçünün tamamlandığını gösteriyor:
   * _`"Normal yürütme modunda en son düğüm"`_ sürümü basıldığında, o zaman yeni **veritabanı** yüklenmiş ve düğüm yeniden başladı.
   * _`"V1.0.0 v1.4.5'ten "göçmen anahtar mağazası"`_ olarak yazıldığında **anahtar** verileri göçmen olarak tamamlanır.
   * _`"V1.0.0 V1.4.5'ten "göçmen platform vm sürümünden v1.0.0 ile v1.4.5 arasında tamamlanmış olarak`_ yazıldığında, onaylayıcı **zamlar** göçü tamamlanır.

Bilgisayarınıza göre güncelleme işlemi önemli bir zaman alabilir. Bazı doğrulayıcılar daha az güçlü bilgisayarlarla 30 saat rapor ettiler. Bu temel olarak bilgisayardaki depolama türüne bağlıdır. Eğer depolama SSD tabanlı ise 12 saat içinde tamamlanmış olur. HDD tabanlı sistemlerde birkaç gün sürebilir (göç neredeyse sadece rastgele okunur ve HDD'ler bu tip iş yüklerinde oldukça yavaştır). Ancak düğümünüz göçler sırasında boş zaman olmadan çalışmaya devam edecek.

Bu sürümün aktarımı için `geçerli` olan sürümünü vererek, verileri verip, [gönderimin](https://docs.avax.network/build/tools/postman-avalanche-collection) tamamlandıktan sonra güncellediğiniz sürüm >=1.4.7 olarak vermeniz gerekir.
...
Eğer bilgisayarınız \(bilgisayar 1\) 6 GB RAM'dan az bir RAM ise, göçü çalıştıramaz çünkü aynı anda 2 düğümlü bir bellek çalıştırmak için yeterli belleği yoktur. Hatırlatmak için your en az 6 GB RAM'ı olduğunu öneriyoruz.

Göç işlemlerini you ve your çevrimdışı olduğu zamanı en aza indirmek istiyorsanız şu şekilde devam edebilirsiniz:

* Başka bir On 2\), AvalancheGo v1.4.5 çalıştır, until kadar bekle, sonra AvalancheGo durdur.
* Bilgisayarda 1, durdurun. Veri tabanı dizini _`$HOME/.avalanchego/db/`_ (daha yeni yüklenmiş veritabanı sürümü v1.4.5\) bilgisayardaki aynı yere taşıyın. Sonra AvalancheGo v1.4.5 ile çalıştırın.
...
Bilgisayarınızda en az 200 GB disk alanı to adamanızı öneriyoruz. AvalancheGo şu anda bu miktarın sadece bir kısmını kullansa da bir delik çözeltisi uygulamadan önce disk kullanımını öngörüyoruz.
...
_`Kullanılan Dosya Sistemi Boyutu Kullanılan % Kullanım`_
...
_`"Normal infaz modunda düğümü çalıştırmaya`_ başladığında" basıldığında, o zaman yeni veritabanı yüklenmiş ve düğüm yeniden başladı.

_`"V1.0.0 v1.4.5'ten "göçmen anahtar mağazası"`_ olarak yazıldığında anahtar verileri göçmen olarak tamamlanır.

_`"V1.0.0 V1.4.5'ten "göçmen platformvm" sürümünden v1.0.0 ile v1.4.5 arasında`_ yazıldığında onaylayıcı süreleri göçmen işlemi biter.
...
_`Home/.avalanchego/db/mainnet/v1.0.`_

Eski veritabanını silmek için gerekli değil. \(v1.0.0\)
...
Muhtemelen hayır. Eğer düğümünüz bir If bu durum daha fazla düşmesine neden olur. \\ (Yukarıdaki cevap\ bkz. Eğer düğümünüz bir doğrulayıcı değilse ama zaten yüklenmiş durumda, veritabanınızı boş bir veritabanından yeniden çizme kayışından geçirmekten daha hızlı olacaktır. Her iki durumda da, mevcut V1.0.0 veritabanını silmektense yukarıda açıklandığı gibi göçü yürütmekten daha iyi olur.
...
İlk olarak, **bu belgeyi iyice okuduğunuzdan emin olun**. Sorunu bir yerde cevaplayabilir. Cevabı göremiyorsanız, [Discord](https://chat.avalabs.org/) sunucumuza gidin ve sorunuzu arayın. Eğer sorulmadıysa, \ #Sorunlu kanala bağlanın.
...
Eğer düğümünüz [installer betiği](https://docs.avax.network/build/tutorials/nodes-and-staking/set-up-node-with-installer) ile kurulduysa, 1.4.5'e yükselttikten sonra servis tanımlama dosyasını onarmanız gerekir. Konsolda, _`the nano /etc/systemd/system/avalanchego.service servisini`_ gir.   Editörde, _`with`_ başlayacak satırı bulun ve şu kısmı silin: _`part:`_ ctrl-x ve y'ye basarak değişiklikleri kaydeder.
