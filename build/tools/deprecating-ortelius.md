...
Esta guía explica cómo sustituir el uso de [la API de Ortelius](ortelius.md) con otras API, incluyendo [la API del Index de AvalancheGo](../avalanchego-apis/index-api.md). **Ortelius es obsoleto. Debes usar otras API de AvalancheGo para obtener la información que antes obtenías de Ortelius.**
...
Puedes usar la API del Index para obtener el número de bloques, vértices o transacciones que ha sido aceptado en una cadena dada. Puedes usar `curl` o el método [getLastAccepted](../avalanchego-apis/index-api.md#index.getlastaccepted) del cliente de la API del Index para lograr esto.

En los ejemplos a continuación, obtendremos el número de transacciones de la X-Chain que han sido aceptadas.
* Para obtener el número de bloques de la C-Chain aceptados, reemplaza `/ext/index/X/tx` con `/ext/index/C/block` a continuación.
* Para obtener el número de bloques de P-Chain aceptados, reemplaza `/ext/index/X/tx` con `/ext/index/P/block`.
* Para obtener el número de vértices de la X-Chain aceptados, reemplaza `/ext/index/X/tx` con `/ext/index/X/vtx`.
...
Puedes usar `curl` para obtener el número de bloques, vértices o transacciones que ha sido aceptado en una cadena dada.
...
(Hay que tomar en cuenta que el campo `bytes` ha sido recortado en aras de la brevedad).
...
## Obtención de una transacción, bloque o vértice por ID
...
Puedes usar el método [avm.getTx](../avalanchego-apis/exchange-chain-x-chain-api.md#avm.gettx) de la API de la X-Chain para obtener una transacción de la X-Chain por su ID. Configura `encoding` a `json` para obtener una respuesta JSON apta para lectura humana.
