...
If you're building an application on the Avalanche network, you may want to do the transfer programmatically as part of some broader functionality. You can do that by calling the appropriate APIs on an AvalancheGo node. The rest of the tutorial assumes you have access to an AvalancheGo node, AVAX tokens on the P-Chain, and user credentials [created](../apis/avalanchego/apis/keystore.md#keystorecreateuser) and stored in the node's keystore.
...
Use the address corresponding to the private key you exported and switch to using the C- prefix in the [`platform.exportAVAX`](../apis/avalanchego/apis/p-chain.md#platformexportavax) call:
...
To finish the transfer, call P-Chain’s [`platform.importAVAX`](../apis/avalanchego/apis/p-chain.md#platformimportavax)
