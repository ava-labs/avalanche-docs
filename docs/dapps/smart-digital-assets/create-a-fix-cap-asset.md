...
Our asset will exist on the [X-Chain](../../overview/getting-started/avalanche-platform-overview.md#exchange-chain-x-chain), so to create our asset we’ll call `avm.createFixedCapAsset`, a method of the [X-Chain’s API](../../apis/avalanchego/apis/x-chain.mdx).
...
To verify this, we call [`avm.getBalance`](../../apis/avalanchego/apis/x-chain.mdx#avmgetbalance):
...
Now, let’s send 100 shares by calling [`avm.send`](../../apis/avalanchego/apis/x-chain.mdx#avmsend).
...
After a second or two, the transaction should be finalized. We can check the status of the transaction with [`avm.getTxStatus`](../../apis/avalanchego/apis/x-chain.mdx#avmgettxstatus):
