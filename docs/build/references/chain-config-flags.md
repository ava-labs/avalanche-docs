...
While ongoing, the pruning process consumes a small amount of additional disk space (for deletion markers and the bloom filter). For more information see [here.](../tutorials/nodes-and-staking/maintain/run-offline-pruning.md#disk-space-considerations)
...
Enables AVM transaction indexing if set to `true`. Default value is `false`. When set to `true`, AVM transactions are indexed against the `address` and `assetID` involved. This data is available via `avm.getAddressTxs` [API](../../build/avalanchego/avalanchego-apis/x-chain.mdx#avmgetaddresstxs).
