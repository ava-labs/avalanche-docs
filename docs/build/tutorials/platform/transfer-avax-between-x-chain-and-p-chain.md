...
You've completed [Getting Started](../avalanchego-node/build/run-avalanche-node-manually.md) and are familiar with [Avalanche's architecture](../../../learn/platform-overview/README.md).
...
If you're building an application on the Avalanche network, you may want to do the transfer programmatically as part of some broader functionality. You can do that by calling the appropriate APIs on an AvalancheGo node. The rest of the tutorial assumes you have access to an AvalancheGo node, AVAX tokens on the X-Chain, and user credentials [created](../../avalanchego/avalanchego-apis/keystore.md#keystorecreateuser) and stored in the node's keystore.
...
To export AVAX, call the X-Chain’s [`avm.export`](../../avalanchego/avalanchego-apis/x-chain.mdx#avmexport) method with `AVAX` assetID.
...
where `to` is the address of a P-Chain address your user controls and `changeAddr` is the address to send any change to. You can leave `changeAddr` blank; if you leave it blank, change will be returned to an address controlled by your user (see [here](../../avalanchego/avalanchego-apis/p-chain.md#platformcreateaddress) for instructions on creating a new P-Chain address).
...
We can verify that this transaction was accepted by calling [`avm.getTxStatus`](../../avalanchego/avalanchego-apis/x-chain.mdx#avmgettxstatus):
...
We can also call [`avm.getBalance`](../../avalanchego/avalanchego-apis/x-chain.mdx#avmgetbalance) to check that the AVAX was deducted from an address held by our user:
...
Our transfer isn’t done just yet. We need to call the P-Chain’s [`platform.importAVAX`](../../avalanchego/avalanchego-apis/p-chain.md#platformimportavax) method to finish the transfer.
...
To do so, call [`platform.exportAVAX`](../../avalanchego/avalanchego-apis/p-chain.md#platformexportavax):
...
This returns the transaction ID, and we can check that the transaction was committed with another call to [`platform.getTxStatus`](../../avalanchego/avalanchego-apis/p-chain.md#platformgettxstatus). Again, make sure that the amount you’re sending exceeds the transaction fee.

### Step 2 - Import AVAX to X-Chain

To finish our transfer from the P-Chain to the X-Chain, call [`avm.import`](../../avalanchego/avalanchego-apis/x-chain.mdx#avmimport):
...
Note that `to` is the same address specified in our call to [`platform.exportAVAX`](../../avalanchego/avalanchego-apis/p-chain.md#platformexportavax).

Just as before, we can call [`avm.getBalance`](../../avalanchego/avalanchego-apis/x-chain.mdx#avmgetbalance) to verify the funds were received. The balance should have increased by `.003` AVAX minus the transaction fee.
...
Now you can use the tokens on the P-Chain to [add a node as a validator](../avalanchego-node/validate/add-a-validator.md) on the Primary Network.
