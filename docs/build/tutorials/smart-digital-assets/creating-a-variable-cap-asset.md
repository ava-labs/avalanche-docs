...
Our asset will exist on the X-Chain, so to create our asset we’ll call [`avm.createVariableCapAsset`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmcreatevariablecapasset), which is a method of the [X-Chain’s API](../../avalanchego/avalanchego-apis//x-chain.mdx).
...
We’ll use [`avm.mint`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmmint) to mint the shares.
...
We can check the status of the transaction we’ve just sent to the network using [`avm.getTxStatus`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmgettxstatus):
...
All 10M shares are controlled by the `to` address we specified in `mint`. To verify this, we’ll use [`avm.getBalance`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmgetbalance):
...
Let’s send 100 shares to another address by using [`avm.send`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmsend). To do so:
