...
You've completed [Run an Avalanche Node](../avalanchego-node/build/run-avalanche-node-manually.md) and are familiar with [Avalanche's architecture](../../../learn/platform-overview/README.md). In this tutorial, we use [Avalanche’s Postman collection](https://github.com/ava-labs/avalanche-postman-collection) to help us make API calls.

## Create the NFT Family

Each NFT belongs to a **family**, which has a name and a symbol. Each family is composed of **groups**. The number of groups in a family is specified when the family is created. Our NFT will exist on the X-Chain, so to create our NFT family we’ll call [`avm.createNFTAsset`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmcreatenftasset), which is a method of the [X-Chain’s API](../../avalanchego/avalanchego-apis//x-chain.mdx).
...
* [`avm.createNFTAsset`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmcreatenftasset)
...
NFT outputs don’t show up in calls to [`avm.getBalance`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmgetbalance) or [`avm.getAllBalances`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmgetallbalances). To see your NFTs, you have to call [`avm.getUTXOs`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmgetutxos) and then parse the utxo to check for the type ID. NFT Mint Outputs have a type id of `00 00 00 0a` in hexidecimal (`10` in decimal) and NFT Transfer Outputs have a type id of `00 00 00 0b` in hexdecimal (`11` in decimal).

### **Method**

* [`avm.getUTXOs`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmgetutxos)
...
[`avm.getUTXOs`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmgetutxos) returns 2 UTXOs. Let’s take the first one and decode it to confirm that it’s an [NFT Mint Output.](../../references/avm-transaction-serialization.md#nft-mint-output) First, we convert the Base58Check encoded string which is returned from [`avm.getUTXOs`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmgetutxos) in to hex. The following [CB58](http://support.avalabs.org/en/articles/4587395-what-is-cb58) string:
...
Now that we have an NFT family and a group for the single `MinterSet` we’re able to create NFTs belonging to this group. To do that we call [`avm.mintNFT`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmmintnft):

### **Method**

* [`avm.mintNFT`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmmintnft)
...
Similar to the previous step, we can now confirm that an NFT was minted by calling [`avm.getUTXOs`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmgetutxos) and parsing the UTXO to confirm that we now have an [NFT Transfer Output](../../references/avm-transaction-serialization.md#nft-transfer-output).
...
First, we convert the Base58Check encoded string which is returned from [`avm.getUTXOs`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmgetutxos) in to hex. The following CB58 string:
...
Now, you can send the NFT to anyone. To do that, use AvalancheGo’s [`avm.sendNFT`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmsendnft) API method.

**Method**

* [`avm.sendNFT`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmsendnft)
...
You can call [`avm.getUTXOs`](../../avalanchego/avalanchego-apis//x-chain.mdx#avmgetutxos) for the address which you sent the NFT to and decompose the returned UTXO, after converting from CB58 to hex, to confirm that there is a UTXO with type id `00 00 00 0b` in hex or `11` in decimal.
