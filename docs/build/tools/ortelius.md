---
description: 'Bu API, istemcilerin Avalanche endeksleyicisi olan Ortelius ile etkileşim kurmalarına imkan verir.'
---
...
**Bu API'nin kullanımdan kaldırılacağı ilan edilmiştir. Nasıl taşınacağını öğrenmek için [buraya](deprecating-ortelius.md) bakın.**

### Format

Bu API, GET HTTP isteklerini URL sorgu parametreleri kullanarak çalıştırır ve JSON verileri döndürür.

### Sürüm Belirleme

Sürüm 2'den itibaren API yollarının önünde bir sürüm etiketi olacaktır, örn. `http://localhost:8080/v2`.

Bu API'nin güncel sürümü, sürüm 2'dir. [Eski API](ortelius.md#legacy-api) dokümanlarında bu API'nin v1 sürümünün kullanımı hakkında bilgiler yer almaktadır.

### Veri Türleri

Tamsayılara, string'lere ve boolean'lara ek olarak, bu API'de şu veri türleri kullanılır:

| Ad | Tarif | Örnekler |
| :--- | :--- | :--- |
| `id` | Bir zincir, işlem veya varlık kimliği gibi CB58 ile kodlanmış bir nesne tanımlayıcı | `2oYMBNV4eNHyqk2fjjV5nVQLDbtmNJzq5s3qs3Lo6ftnC6FByM` |
| `address` | Bech-32 ile kodlanmış bir adres \(X-Chain ve P-Chain'de kullanılır\) | `fuji1wycv8n7d2fg9aq6unp23pnj4q0arv03ysya8jw` |
| `datetime` | Bir tamsayı veya bir RFC3339 biçimli string şeklinde bir Unix zaman damgası | `1599696000`, `2020-09-10T00:00:00Z` |
| `caddress` | Hex \(on altılık sayı sistemi\) ile kodlanmış bir adres \(C-Chain'de kullanılır\) | `0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7` |
| `chash` | Hex ile kodlanmış bir hash \(kripto özütü\) | `0xe5b935988317e8552e769ad92b6a2fd01ac8f0f90d8ffa4377e50fcb8d970077` |

### Liste Parametreleri

Kaynakların listelenmesine ilişkin tüm son noktalar aşağıdaki parametreleri kabul eder:

| Ad | Tür | Tarif | Varsayılan | Maksimum |
| :--- | :--- | :--- | :--- | :--- |
| `limit` | `int` | Döndürülecek ögelerin maksimum sayısı | `5000` | `5000` |
| `query` | `string` | Ögelerin filtreleneceği bir kimlik ön eki | Yok | Yok |
| `startTime` | `datetime` | Belli bir zamanda veya sonrasında yaratılan ögelere uygulanan limitler | `0` | Şimdi |
| `endTime` | `datetime` | Belli bir zamanda veya öncesinde yaratılan ögelere uygulanan limitler | Şimdi | Şimdi |

## Mevcut Son Noktalar

### Genel Bakış

API'nin kökü, endekslenen aktif Avalanche ağına ait sabit değerlerin \(constant\) genel bir görünümünü verir.
...
ID kullanarak bir adres veya işlemi bulun.
...
### Topak Veriler \(Aggregate\)

Bir zaman aralığı içindeki topak işlem verilerinin hesaplanması.

**Parametreler**

| Ad | Tür | Tarif | Varsayılan | Maksimum |
| :--- | :--- | :--- | :--- | :--- |
| `chainID` | `id` | Sonuçların filtrelenmesinde kullanılan bir zincir kimliği. Defalarca verilebilir. | Yok | Yok |
| `assetID` | `id` | Sonuçların filtrelenmesinde kullanılan bir varlık kimliği. | Yok | Yok |
| `intervalSize` | `string` | Değerler 'dakika', 'saat', 'gün', 'hafta', 'ay', 'yıl' | Yok | Yok |
...
Bir adresin üzerinde göründüğü zincirlerle yanıt verir.

**Parametreler**

| Ad | Tür | Tarif | Varsayılan | Maksimum |
| :--- | :--- | :--- | :--- | :--- |
| `address` | `address` | Sonuçların filtrelenmesinde kullanılan bir adres. Defalarca verilebilir. | Yok | Yok |
...
Kabul edilmiş olan işlemleri bulun.
...
| `assetID` | `id` | Sonuçların filtrelenmesinde kullanılan bir varlık kimliği. | Yok | Yok |
| `address` | `address` | Sonuçların filtrelenmesinde kullanılan bir adres. Defalarca verilebilir. | Yok | Yok |
| `sort` | `string` | Sonuçların sıralanmasında kullanılan bir yöntem. `timestamp-asc` veya  `timestamp-desc`olabilir. | `timestamp-asc` | İlgili Değil |
...
İşlemin kimliğini kullanarak tek bir işlemi bulun.
...
Kabul edilen işlemlere referanslı adresleri bulun.
...
Adresin kimliğini kullanarak tek bir adresi bulun.
...
X-chain platformunda yaratılmış varlıkları bulun.
...
Varlığın kimliğini kullanarak tek bir varlığı bulun.
...
Kabul edilmiş bir işlemin ürettiği çıktıları bulun.
...
| `chainID` | `id` | Sonuçların filtrelenmesinde kullanılan bir zincir kimliği. Defalarca verilebilir. | Yok | Yok |
| `address` | `address` | Sonuçların filtrelenmesinde kullanılan bir adres. Defalarca verilebilir. | Yok | Yok |
| `spent` | `bool` | Ayarlanırsa, sonuçlar harcanmış \(true\) veya harcanmamış \(false\) olup olmamalarına göre filtrelenir | Yok | İlgili Değil |
...
Tekil bir çıktıyı onun kimliğini kullanarak bulun.
...
### Bir C-Chain Blokunu Getirme

Tekil bir bloku onun numarasını kullanarak bulun.
...
### C-Chain İşlemlerini Listeleme

Kabul edilmiş C-Chain işlemlerini bulun.

**Parametreler**

| Ad | Tür | Tarif | Varsayılan | Maksimum |
| :--- | :--- | :--- | :--- | :--- |
| `toAddress` | `caddress` | adres | Yok | Yok |
| `fromAddress` | `caddress` | adres | Yok | Yok |
| `address` | `caddress` | adres \(...'e veya ...'den\) | Yok | Yok |
| `hash` | `chash` | işlem hash'i | Yok | Yok |
| `blockStart` | `number` | Başlangıç blok numarası dahil | Yok | İlgili Değil |
| `blockEnd` | `number` | Bitiş blok numarası hariç | Yok | İlgili Değil |
...
### Bir Ham C-Chain İşlemini Getirme

Kabul edilmiş bir C-Chain işlemini onun kimliğini kullanarak getirin.
...
Bu API'nin 1'inci sürümü sadece X-chain platformunu desteklenmek için oluşturulmuştu ve bir sürüm ön eki \(`/v1`\) almamıştı. Bu sürüm, kökün `/x` yolunda bulunur ve sadece X-chain için Genel Bakış son noktasıdır:
...
Eski API, sürüm 2'deki aynı son noktaları ve parametreleri destekler; bunun bir istisnası, X-Chain ID'ye atanmış tüm son nokta varsayılanlarına ait zincir kimliği parametresidir.
