...
These are pre-configured Grafana dashboards that work with the setup as shown in [Setting up node monitoring](../../tutorials/avalanchego-node/maintain/setting-up-node-monitoring.md). The script in the linked tutorial will automatically install them.
