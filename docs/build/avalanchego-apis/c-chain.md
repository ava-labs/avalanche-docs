...
This API can be used for debugging. Note that the Admin API is disabled by default. To run a node with the Admin API enabled, use [config flag argument](../references/avalanchego-config-flags.md#c-chain-config) `--coreth-admin-api-enabled:true`.
