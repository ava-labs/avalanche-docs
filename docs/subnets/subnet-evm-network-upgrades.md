...
## Upgrade: Enable/Disable Precompiles
...
[`config.json`](../subnets/subnet-evm-chain-configs) resides: `{chain-config-dir}/{blockchainID}/upgrade.json`.
...
See every precompile initial configuration in their relevant `Initial Configuration` sections under [Precompiles](../subnets/subnet-evm-precompiles).
