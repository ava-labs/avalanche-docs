...
Let's see some of the currently precompiles interfaces available:

- Transaction Allow List
- SmartContract Deployer Allow List
- Fee Config
- Fee reward mechanism
- Allow fee recipient
...
- `setFeeConfig` - sets the dynamic fee config on chain (see 
[here](../subnets/subnet-evm-genesis-file.md#fee-config) 
...
- `allowFeeRecipients` - enables block producers to claim fees. This will allow block producers to
  claim fees by specifying their own addresses in their chain configs. 
  See [here](../subnets/customize-a-subnet#changing-fee-reward-mechanisms) to modify fee recipients
  via a precompile or [here](../subnets/customize-a-subnet#changing-fee-reward-mechanisms) to
  configure it via a genesis file. 
