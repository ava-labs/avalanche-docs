...
First, let’s generate the 2 control keys. To do so we call [`platform.createAddress`](../apis/avalanchego/apis/p-chain.md#platformcreateaddress) This generates a new private key and stores it for a user.
...
To create a subnet, we call [`platform.createSubnet`](../apis/avalanchego/apis/p-chain.md#platformcreatesubnet).
...
We can call [`platform.getSubnets`](../apis/avalanchego/apis/p-chain.md#platformgetsubnets) to get all Subnets that exist:
...
To add the validator, we’ll call API method [`platform.addSubnetValidator`](../apis/avalanchego/apis/p-chain.md#platformaddsubnetvalidator). Its signature is:
...
We can check the transaction’s status by calling [`platform.getTxStatus`](../apis/avalanchego/apis/p-chain.md#platformgettxstatus):
...
For more information about the command see: [whitelisted-subnet command-line argument](../nodes/maintain/avalanchego-config-flags.md#whitelist).
...
Subnet validators/beacons can choose not to publish contents of blockchains via an optional `validatorOnly` configuration. The configuration can be turned on with [Subnet Configs](../nodes/maintain/avalanchego-config-flags.md#subnet-configs). If a node sets `validatorOnly` to `true`, the node exchanges messages only with this subnet's validators. Other peers will not be able to learn contents of this subnet from this node.
