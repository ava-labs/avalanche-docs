...
This file is meant to be the single source of truth for how we serialize transactions in Avalanche’s Platform Virtual Machine, aka the `Platform Chain` or `P-Chain`. This document uses the [primitive serialization](platform-transaction-format.md) format for packing and [secp256k1](platform-transaction-format.md) for cryptographic user identification.
...
A [secp256k1](platform-transaction-format.md) transfer output allows for sending a quantity of an asset to a collection of addresses after a specified unix time. The only valid asset is AVAX.
...
A [secp256k1](platform-transaction-format.md) output owners output will recieve the staking rewards when the lock up period ends.
...
A [secp256k1](platform-transaction-format.md) transfer input allows for spending an unspent secp256k1 transfer output.
...
A [secp256k1](platform-transaction-format.md) credential contains a list of 65-byte recoverable signatures.
