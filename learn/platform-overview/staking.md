...
バリデータは、オンラインかつバリデーション時間の80%を超えて応答している場合、ステーキング報酬を受け取ります。これは、ステーキングによって重みづけされたバリデータの大半によって測定されます。**バリデータがオンラインでかつ100%の時間応答していることを目指してください。**

ノード上のAPIメソッド`info.uptime`を呼び出し、重みづけられたアップタイムと、現在ノードがステーキング報酬を受け取るに十分なアップタイムを維持しているとみなしているネットワークの割合を知ることができます。[こちら](../../build/avalanchego-apis/info-api.md#info-uptime)を参照してください。Avalancheの[ステーキングダッシュボード](https://stats.avax.network/dashboard/staking/)からノードのアップタイムについて別の意見を取得することができます。報告されたアップタイムが100%近くでない場合、ノードの設定に問題があり、ステーキング報酬を損なう可能性があります。このような場合、[こちら](#why-is-my-uptime-low)を参照するか、[Discord](https://chat.avax.network)からご連絡ください。問題についてのサポートを行います。ステーキングノード以外のノード、少量のステークのバリデータあるいはバリデーション期間中ずっとオンラインではないバリデータにより測定されたバリデータのアップタイムをチェックするだけでは、ノードの本当のアップタイムを正確に確認できない場合があります。

## バリデーター

**バリデータ**は、Avalancheを保護し、新しいブロック／バーテックスを作成し、トランザクションを処理します。コンセンサスに達するため、バリデーターは、互いに繰り返しサンプリングを行います。特定のバリデーターがサンプリングされる確率は、そのステーク量に比例します。
...
Avalanche上の全てのバリデーターは、他のバリデーターの稼働時間を追跡します。すべてのバリデータには重みがあります。（ステークされた金額など）バリデータの重みが大きいほど、ノードがステーキング報酬を受け取るか否かについてバリデータが投票した際より大きな影響を持つことになります。ノード上のAPIメソッド`info.uptime`を呼び出し、重みづけられたアップタイムと、現在ノードがステーキング報酬を受け取るに十分なアップタイムを維持しているとみなしているネットワークの割合を知ることができます。

また、`info.peers`を呼び出すことでノードの接続を確認したり、各接続のアップタイムを確認することができます。**これはあくまでも一つのノードの視点に過ぎません**。他のノードは、あなたのノードのアップタイムを異なる形で認識する可能性もあります。あるノードがあなたのアップタイムが低いと認識しても、ステーク報酬を受け取れなくなるということではありません。

ノードのアップタイムが低い場合は、必ず構成オプション`--public-ip=[NODE'S PUBLIC IP]`を設定し、ノードがポート9651でTCPトラフィックを受信できるようにしてください。
...
はい、[こちら](https://stats.avax.network/dashboard/validator-health-check)にはあなたのノードIDを入力してください。
