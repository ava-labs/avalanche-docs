...
contract('Marketplace', ([contractDeployer, creator, buyer, secondBuyer]) => {
  ...
  before(async () => {
    ...
    await marketplace.createCollectible('metadata', 20, {
      from: creator
    });
  });
  ...
  describe('List a NFT.', async () => {
    ...
    it('The NFT with token id 1 cannot be listed by anyone who doesn\'t own it.', async () => {
      await expectRevert(marketplace.listItem(1, convertTokensToWei('5'), {
        from: contractDeployer
    ...
    it('Transfer the NFT to the Marketplace SC.', async () => {
      const result = await marketplace.listItem(1, convertTokensToWei('5'), {
        from: creator
      });
      assert.equal(result.logs.length, 3, 'Should trigger three events.'); //event Approval
      ...
      assert.equal(result.logs[0].args.tokenId, 1, 'Should log the token id which is 1.'); //event Transfer
      ...
      assert.equal(result.logs[1].args.tokenId, 1, 'Should log the token id which is 1.'); //event ItemListed
    ...
    it('The NFT with token id 1 cannot be listed again.', async () => {
      await expectRevert.unspecified(marketplace.listItem(1, convertTokensToWei('5'), {
        from: creator
      }));
    });
  ...
  describe('Cancel the listing.', async () => {
    it('The listing cannot be cancelled by an address that does not have the right to claim it.', async () => {
      await expectRevert(marketplace.cancelListing(1, {
        from: contractDeployer
    ...
    it('Transfer the NFT back to the owner.', async () => {
      const result = await marketplace.cancelListing(1, {
        from: creator
      });
      assert.equal(result.logs.length, 3, 'Should trigger three events.'); //event Approval
      ...
      assert.equal(result.logs[1].args.tokenId, 1, 'Should log the token id which is 1.'); //event ListingCancelled
  ...
  describe('Buy a NFT.', async () => {
    ...
    it('Buy the NFT.', async () => {
      ...
      const result = await marketplace.buyItem(1, {
        from: buyer,
        value: convertTokensToWei('5')
      });
      assert.equal(result.logs.length, 3, 'Should trigger three events.'); //event Approval
      ...
      assert.equal(result.logs[1].args.tokenId, 1, 'Should log the token id which is 1.'); //event ItemBought
    ...
    it('The balances of creator, first buyer who is now the seller and second buyer are correct.', async () => {
      ...
      await marketplace.listItem(1, convertTokensToWei('10'), {
        from: buyer
      });
